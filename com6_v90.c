/*
 * COM6 v90 - Deep KC for 8192 + Reduced C-Passes
 * ================================================
 * Built on v89's L1-safe foundation. Key change: for n>=8192,
 * deepen KC from 320→384 to cut DRAM C-passes from 26→22 (15% fewer).
 *
 * L1 safety check: A micro-tile = MR*KC*8 = 6*384*8 = 18.4KB < 32KB L1D ✓
 * L2 fit: MC*KC*8 = 80*384*8 = 245KB < 256KB L2 ✓
 * L3 fit: NC*KC*8 = 2048*384*8 = 6.3MB < 8MB L3 (1.7MB headroom) ✓
 *
 * Key changes from v89:
 * 1. KC=384/MC=80 for n>=8192 (fewer C-passes = less DRAM bandwidth)
 * 2. KC_MAX bumped to 384 (was 320)
 * 3. All other sizes unchanged (v89's blocking proven optimal)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v90 com6_v90.c -lm
 */
...
/* n>=8192: moderately deeper KC to reduce C-passes.
 * A micro-tile = 6*384*8=18.4KB fits L1D (32KB) ✓
 * MC*KC = 80*384*8=245KB fits L2 (256KB) ✓
 * NC*KC = 2048*384*8=6.3MB fits L3 (8MB) with 1.7MB headroom ✓ */
#define KC_HUGE  384
#define MC_HUGE  80     /* 80*384*8=245KB (fits L2 256KB) */
#define NC_HUGE  2048   /* 384*2048*8=6.3MB (fits 8MB L3) */

#define KC_MAX 384      /* Max of all KC tiers */
#define MC_MAX 120
#define NC_MAX 2048
...
/* Pin threads to physical cores 0-3 (even-numbered logical CPUs on HT systems)
 * This prevents OS from scheduling two threads on the same physical core */
static void pin_threads_to_physical_cores(int nthreads){
#ifdef _WIN32
    #pragma omp parallel num_threads(nthreads)
    {
        int tid = omp_get_thread_num();
        /* On HT systems, physical cores are logical CPUs 0,2,4,6 (even) */
        DWORD_PTR mask = (DWORD_PTR)1 << (tid * 2);
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* Pin to physical cores to avoid HT sibling scheduling */
    if(nthreads <= 4) pin_threads_to_physical_cores(nthreads);
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v90 - Deep KC for 8192 (%d threads)\n",nth);
    printf("  n<=1024: MC=%d KC=%d | n<=4096: MC=%d KC=%d\n",MC_MT_SMALL,KC_SMALL,MC_LARGE,KC_LARGE);
    printf("  n>=8192: MC=%d KC=%d (A-tile 18KB, 22 C-passes vs 26)\n",MC_HUGE,KC_HUGE);
    ...
    printf("\nv90: Deep KC=384 for 8192+ (22 C-passes vs 26), 1.7MB L3 headroom.\n");
    printf("Run individual: ./com6_v90 <size> [mt|1t]\n");
    return 0;
}
