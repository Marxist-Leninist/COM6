/*
 * COM6 v116 - Persistent OMP pool: bounded active wait + proc bind
 * =====================================================================
 * Heritage:
 *   v108 dispatch, blocking, micro-kernel, and pacing — UNCHANGED.
 *   The only difference is runtime configuration of libgomp before
 *   the first parallel region, plus an explicit pool-warmup helper.
 *
 * Problem we are addressing:
 *   The README v80 row "OpenBLAS MT 105.1 GF vs COM6 v80 73.9 GF
 *   (0.70x)" at 512 is the most-cited COM6 weakness. The kernel and
 *   blocking are equivalent to OpenBLAS at that size, so the gap has
 *   to come from dispatch — specifically OpenMP thread wake-up.
 *
 *   v107/v108 already collapsed the parallel region to one fork/join
 *   per matmul. What's left is: (a) the very first call of a benchmark
 *   pays cold-pool cost; (b) calls separated by a Sleep() see workers
 *   futex-sleep in libgomp's default OMP_WAIT_POLICY=passive; (c) the
 *   OS scheduler can migrate workers between cores between calls.
 *
 * What v116 does (purely runtime hygiene — no kernel changes):
 *
 *   1. configure_omp_runtime() runs at the very top of main() and sets
 *      env vars libgomp parses on first parallel-region init:
 *
 *        OMP_WAIT_POLICY=active     spin briefly between regions
 *        GOMP_SPINCOUNT=200000      ~65 us at 3 GHz before sleeping —
 *                                   bridges back-to-back regions but
 *                                   workers DO sleep on multi-ms idle,
 *                                   so the 4 s inter-size cool-down
 *                                   does not burn thermal budget.
 *        OMP_PROC_BIND=close        keep team threads on nearby cores
 *        OMP_PLACES=cores           one place per physical core preferred
 *        GOMP_CPU_AFFINITY=0 2 4 6 1 3 5 7
 *                                   HT-aware: workers 0..3 land on
 *                                   distinct physical cores; 4..7 share
 *                                   HT siblings only when needed.
 *        OMP_DYNAMIC=false          fix team size
 *
 *      env_set_if_unset() preserves any user-supplied value, so the
 *      caller can still override (OMP_NUM_THREADS=4 ./com6_v116 …).
 *
 *      CRITICAL: GOMP_SPINCOUNT is NEVER set to "infinite" — that pegs
 *      idle workers at 100% CPU during inter-call pauses, which on a
 *      15W TDP laptop scorches the thermal budget and ruins the next
 *      timed call. Empirically tested: infinite produced 8-9 GF runs
 *      at 512 MT after the first warm one (vs ~60 GF baseline).
 *
 *   2. omp_warmup_pool() spawns a brief parallel region whose body is
 *      a small atomic increment loop. Called once at startup and
 *      again after each inter-size Sleep() in the suite. Cost is
 *      ~2-3 ms; it puts every worker into libgomp's spin-wait so the
 *      next user-facing call has clean dispatch.
 *
 * Env switches (in addition to all v108 envs which still apply):
 *   COM6_NO_OMP_TUNING=1   skip configure_omp_runtime() (revert to v108)
 *   COM6_NO_WARMUP=1       skip warmup spins
 *
 * Why a separate version (not a v115 successor): the user has an
 * uncommitted v115 in the working tree pursuing size-adaptive HT
 * pinning (different intervention). v116 keeps OMP-runtime hygiene
 * cleanly isolated and orthogonal — the two changes can later be
 * combined if both prove out.
 *
 * Build:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v116 com6_v116.c -lm
 */
...
/* ================================================================
 * v116: persistent OpenMP pool helpers (runtime-only, no kernel diff)
 * ================================================================ */
#ifndef _WIN32
extern int setenv(const char*, const char*, int);
#endif

/* Set env var only if not already set by the caller. Portable across
 * MSYS2/MinGW (which lacks POSIX setenv without _GNU_SOURCE) and POSIX. */
static int env_set_if_unset(const char* name, const char* value){
    const char* cur = getenv(name);
    if(cur && cur[0]) return 0;  /* respect user-supplied value */
#ifdef _WIN32
    char buf[256];
...
/* Configure libgomp BEFORE any omp_* call. Idempotent / safe to repeat,
 * but only the FIRST invocation before the first parallel region wins
 * (libgomp caches its config when the team is first created). */
static void configure_omp_runtime(void){
    if(getenv("COM6_NO_OMP_TUNING")) return;

    /* Active wait between parallel regions, with bounded spin so idle
     * workers eventually sleep (critical for thermal budget on a 15W
     * laptop). 200000 cycles ~ 65 us at 3 GHz — bridges back-to-back
     * regions inside one matmul, lets workers sleep on Sleep()s. */
    env_set_if_unset("OMP_WAIT_POLICY", "active");
    env_set_if_unset("GOMP_SPINCOUNT",  "200000");

    /* Pin threads to nearby cores; don't let the OS migrate them. */
    env_set_if_unset("OMP_PROC_BIND", "close");
    env_set_if_unset("OMP_PLACES",    "cores");

    /* HT-aware affinity for i7-10510U topology (4 cores / 8 threads,
     * physical 0/2/4/6, HT siblings 1/3/5/7). Worker N maps to the
     * Nth listed CPU. Harmless on machines with different topology
     * because libgomp wraps modulo the actual logical-CPU count. */
    env_set_if_unset("GOMP_CPU_AFFINITY", "0 2 4 6 1 3 5 7");

    /* Fix team size — don't let runtime opportunistically shrink it. */
    env_set_if_unset("OMP_DYNAMIC", "false");
}

/* Force libgomp to spawn workers and put them in the active spin loop.
 * The body is a trivial atomic to defeat dead-code elimination. */
static void omp_warmup_pool(void){
    if(getenv("COM6_NO_WARMUP")) return;
#ifdef _OPENMP
    volatile int spin_sink = 0;
    /* First region: forces team creation + initial proc-bind setup. */
    #pragma omp parallel
    {
        #pragma omp atomic
        spin_sink++;
    }
    /* Second region: short busy work per thread. By the time this
     * returns every worker has executed once and is in libgomp's
     * spin-wait loop with caches warm for parallel-region overhead. */
    #pragma omp parallel
    {
        volatile long acc = 0;
        for(long i = 0; i < 200000L; i++) acc += i;
        if(acc == 0) spin_sink++;
    }
    (void)spin_sink;
#endif
...
/* v103: Persistent packing-buffer context shared across Strassen sub-muls */
typedef struct {
    double** pa_bufs;     /* [nthreads] each MC_MAX*KC_MAX doubles */
    double*  pb;          /* KC_MAX*NC doubles (IC-par) */
    double** pb_jc;       /* [nthreads] KC_MAX*NC_MAX (JC-par) */
    double** pa_jc;       /* [nthreads] MC_MAX*KC_MAX (JC-par) */
    int      nthreads;
    int      owns;        /* 1 if we must free on release */
} com6_ctx_t;

static com6_ctx_t* com6_ctx_alloc(int nthreads){
    com6_ctx_t* c = (com6_ctx_t*)malloc(sizeof(com6_ctx_t));
    c->nthreads = nthreads;
    c->owns = 1;
    c->pa_bufs = (double**)malloc(nthreads * sizeof(double*));
    c->pa_jc   = (double**)malloc(nthreads * sizeof(double*));
    c->pb_jc   = (double**)malloc(nthreads * sizeof(double*));
    for(int t=0;t<nthreads;t++){
        c->pa_bufs[t] = aa((size_t)MC_MAX*KC_MAX);
        c->pa_jc[t]   = aa((size_t)MC_MAX*KC_MAX);
        c->pb_jc[t]   = aa((size_t)KC_MAX*2048);  /* NC_MAX */
    }
    c->pb = aa((size_t)KC_MAX*2048);               /* NC */
    return c;
}
static void com6_ctx_free(com6_ctx_t* c){
    if(!c || !c->owns) return;
    for(int t=0;t<c->nthreads;t++){
        af(c->pa_bufs[t]); af(c->pa_jc[t]); af(c->pb_jc[t]);
    }
    free(c->pa_bufs); free(c->pa_jc); free(c->pb_jc);
    af(c->pb);
    free(c);
}
...
static void get_blocking_mt(int n, int*pMC, int*pKC){
    /* v107: revert to v106 values; improvement is in IC schedule, not MC.
     * MC_MT_BAL=64 experiment failed because 64*256=128KB = HT-shared L2. */
...
/* v103: JC-parallel with pre-allocated per-thread buffers from ctx */
static void com6_multiply_jc_ctx(const double*__restrict__ A,
                                  const double*__restrict__ B,
                                  double*__restrict__ C, int n,
                                  int mc_blk, int kc_blk,
                                  com6_ctx_t* ctx)
{
    #pragma omp parallel
    {
        ...
        if(j_start < n){
            double*pa = ctx->pa_jc[tid];
            double*pb = ctx->pb_jc[tid];
...
/* out = X + Y (both hxh contiguous) -- parallel over rows */
static void mat_add(const double*X, const double*Y, double*out, int h){
    #pragma omp parallel for schedule(static)
    for(int i=0;i<h;i++){
        size_t off=(size_t)i*h;
        const double*x=X+off,*y=Y+off;double*o=out+off;
        size_t j=0;
        for(;j+3<(size_t)h;j+=4){
            __m256d a=_mm256_loadu_pd(x+j), b=_mm256_loadu_pd(y+j);
            _mm256_storeu_pd(o+j,_mm256_add_pd(a,b));
        }
        for(;j<(size_t)h;j++) o[j]=x[j]+y[j];
    }
}
...
static void mat_sub(const double*X, const double*Y, double*out, int h){
    ...
    for(int i=0;i<h;i++){
        ...
        for(;j+3<(size_t)h;j+=4){
            __m256d a=_mm256_loadu_pd(x+j), b=_mm256_loadu_pd(y+j);
            _mm256_storeu_pd(o+j,_mm256_sub_pd(a,b));
        }
        for(;j<(size_t)h;j++) o[j]=x[j]-y[j];
    }
}
...
/* Write: Csub[r..r+h, c..c+h] = M (contiguous hxh) -- parallel over rows */
...
static void com6_multiply(const double*__restrict__ A,
                           const double*__restrict__ B,
                           double*__restrict__ C,int n);
static void com6_multiply_ctx(const double*__restrict__ A,
                               const double*__restrict__ B,
                               double*__restrict__ C, int n,
                               com6_ctx_t* ctx);
...
static void com6_strassen(const double*A, const double*B, double*C, int n){
    ...
    /* v101: Single pool allocation for all 23 temp buffers.
     * Reduces malloc churn, improves TLB locality, and pages are typically
     * already resident from the previous sub-mul when BLIS reads them. */
    const int NBUF = 23;  /* 8 inputs + 8 sums + 7 products */
    double *pool = aa((size_t)NBUF * hh);
    #define P(i) (pool + (size_t)(i) * hh)
    double *A11=P(0),*A12=P(1),*A21=P(2),*A22=P(3);
    double *B11=P(4),*B12=P(5),*B21=P(6),*B22=P(7);
    double *S1 =P(8),*S2 =P(9),*S3 =P(10),*S4 =P(11);
    double *T1 =P(12),*T2=P(13),*T3=P(14),*T4=P(15);
    double *M1 =P(16),*M2=P(17),*M3=P(18),*M4=P(19);
    double *M5 =P(20),*M6=P(21),*M7=P(22);
    ...
    /* v103: allocate packing buffers ONCE, reuse across all 7 sub-muls.
     * Eliminates 7 * (nthreads+1) alloc/free rounds and keeps pa/pb warm. */
    int nthreads_strassen = 1;
    #ifdef _OPENMP
    nthreads_strassen = omp_get_max_threads();
    #endif
    com6_ctx_t* ctx = com6_ctx_alloc(nthreads_strassen);

    /* v105: thermal pacing between sub-muls */
    int pace_ms = 500;
    const char *env = getenv("COM6_STRASSEN_PACE_MS");
    if(env){ int v = atoi(env); if(v >= 0 && v <= 10000) pace_ms = v; }

    com6_multiply_ctx(A11, B11, M1, h, ctx);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply_ctx(A12, B21, M2, h, ctx);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply_ctx(S4,  B22, M3, h, ctx);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply_ctx(A22, T4,  M4, h, ctx);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply_ctx(S1,  T1,  M5, h, ctx);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply_ctx(S2,  T2,  M6, h, ctx);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply_ctx(S3,  T3,  M7, h, ctx);

    com6_ctx_free(ctx);

    /* Reuse S1,S2 as U and tmp scratch (inputs no longer needed) */
    double *U=S1, *tmp=S2;

    mat_add(M1, M6, U, h);               /* U = M1+M6 */

    mat_add(M1, M2, tmp, h);             /* C11 = M1+M2 */
    sub_write(C, n, 0, 0, h, tmp);

    mat_add(U, M5, tmp, h);              /* C12 = U+M5+M3 */
    mat_add(tmp, M3, tmp, h);
    sub_write(C, n, 0, h, h, tmp);

    mat_add(U, M7, tmp, h);              /* C21 = U+M7-M4 */
    mat_sub(tmp, M4, tmp, h);
    sub_write(C, n, h, 0, h, tmp);

    mat_add(U, M7, tmp, h);              /* C22 = U+M7+M5 */
...
/* v103: IC-parallel body using externally provided pa_bufs/pb (shared)
 * v106: pace_ms argument — Sleep between NC column blocks for thermal recovery
 * (only effective when n > NC, i.e., multi-block execution). */
static void com6_multiply_ic_body(const double*__restrict__ A,
                                   const double*__restrict__ B,
                                   double*__restrict__ C, int n,
                                   int mc_blk, int kc_blk,
                                   double** pa_bufs, double* pb,
                                   int pace_ms)
{
    /* v107: single hoisted parallel region for whole matmul */
    #pragma omp parallel
    {
        int tid = omp_get_thread_num();
        int nt = omp_get_num_threads();
        ...
        for(int jc=0;jc<n;jc+=NC){
            ...
            /* Pace between column blocks (master thread only, then barrier) */
            if(jc > 0 && pace_ms > 0){
                #pragma omp master
                {
#ifdef _WIN32
            ...
            for(int pc=0;pc<n;pc+=kc_blk){
                ...
                /* IC loop: implicit barrier at end of omp-for */
...
/* IC-parallel (shared B, divided ic) — allocates buffers itself */
                              ...
                              double*__restrict__ C, int n)
{
    ...
    /* v108: pacing extended to n >= 4096 (was 8192 in v107).
    ...
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,pa_bufs,pb,pace_ms);

    for(int t=0;t<nthreads;t++) af(pa_bufs[t]);
    free(pa_bufs); af(pb);
}

/* v103: IC-parallel using persistent ctx buffers */
static void com6_multiply_ic_ctx(const double*__restrict__ A,
                                  const double*__restrict__ B,
                                  double*__restrict__ C, int n,
                                  com6_ctx_t* ctx)
{
    int mc_blk, kc_blk;
    get_blocking_mt(n, &mc_blk, &kc_blk);
    /* ctx path used inside Strassen sub-muls where pacing is handled externally */
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,ctx->pa_bufs,ctx->pb,0);
}

/* v103: ctx-aware dispatch — same as com6_multiply but reuses ctx buffers */
static void com6_multiply_ctx(const double*__restrict__ A,
                               const double*__restrict__ B,
                               double*__restrict__ C, int n,
                               com6_ctx_t* ctx)
{
    ...
    /* Inside Strassen: n here is h = outer_n/2, so typically 4096 for outer=8192.
     * Route by the same per-size policy as the top-level dispatch. */
    if(n >= 4096){
        com6_multiply_ic_ctx(A, B, C, n, ctx);
    } else if(n >= 2048){
        int mc_blk, kc_blk;
        get_blocking(n, &mc_blk, &kc_blk);
        com6_multiply_jc_ctx(A, B, C, n, mc_blk, kc_blk, ctx);
    } else {
        com6_multiply_ic_ctx(A, B, C, n, ctx);
    }
}
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* v106: at 8192, IC-par with per-NC pacing beats Strassen+pacing.
     * Strassen is only used if COM6_USE_STRASSEN=1 (for comparison). */
    ...
    /* v108: JC-parallel at 2048 (v107 default — wins in warm-CPU tests).
     * COM6_IC_2048=1 opts into IC-par for 2048 as an experiment hook. */
    if(n >= 4096){
        com6_multiply_ic(A, B, C, n);
    } else if(n >= 2048){
        const char* ic_env = getenv("COM6_IC_2048");
        if(ic_env && atoi(ic_env) == 1){
            com6_multiply_ic(A, B, C, n);
        } else {
            int mc_blk, kc_blk;
            get_blocking(n, &mc_blk, &kc_blk);
...
int main(int argc, char**argv){
    /* v116: configure libgomp BEFORE the first omp_* call below.
     * env_set_if_unset preserves user-supplied OMP_* values. */
    ...
    /* Spawn worker threads now and put them in the spin loop. The cost
     * (~2-3 ms) is paid once at startup, NOT inside any timed call. */
    omp_warmup_pool();
    ...
    if(argc >= 2){
        ...
        if(mode!=1){
            /* v116: extra warmup right before MT timing — covers the
             * case where the 1T path above didn't touch the OMP pool. */
            omp_warmup_pool();
    ...
    printf("====================================================================\n");
    printf("  COM6 v116 - Persistent OMP pool + bounded active wait (%d threads)\n",nth);
    printf("  Dispatch UNCHANGED from v108 (8192/4096 IC-par+pace | 2048 JC-par | <=1024 IC-par)\n");
    printf("  1T: MC=%d/%d KC=%d/%d | NC_JC=%d | NC=%d\n",
           MC_SMALL,MC_LARGE,KC_SMALL,KC_LARGE,NC_JC,NC);
    printf("  OMP_WAIT_POLICY=%s  GOMP_SPINCOUNT=%s  OMP_PROC_BIND=%s\n",
           getenv("OMP_WAIT_POLICY")?:"(unset)",
           getenv("GOMP_SPINCOUNT")?:"(unset)",
           getenv("OMP_PROC_BIND")?:"(unset)");
    printf("  Reverse order: 8192 first (cold CPU). Per-size pool re-warm after Sleep.\n");
    printf("====================================================================\n\n");
    ...
    for(int si=0;si<ns;si++){
        ...
        if(si > 0){
            ...
            Sleep(4000);
#endif
            /* v116: re-warm the pool after the thermal cool-down. The
             * 4 s sleep lets workers fall back to libgomp futex sleep
             * (because GOMP_SPINCOUNT=200000 means ~65 us spin then
             * sleep). Re-warm puts them back in active wait so the
             * next benchmarked call has clean dispatch. */
            omp_warmup_pool();
        }
    ...
    printf("\nv116: persistent OMP pool with bounded active wait + HT-aware proc bind.\n");
    printf("Dispatch identical to v108: 8192/4096 IC-par+pace | 2048 JC-par | <=1024 IC-par.\n");
    printf("COM6_NO_OMP_TUNING=1 reverts to v108 runtime behavior (regression test).\n");
    printf("COM6_NO_WARMUP=1 skips startup/per-size warmup spins.\n");
    printf("COM6_PACE_MS=N | COM6_PACE_MIN_N=N | COM6_IC_2048=1 | COM6_USE_STRASSEN=1 still apply.\n");
    printf("Run individual: ./com6_v116 <size> [mt|1t]\n");
    return 0;
}
