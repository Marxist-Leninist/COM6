/*
 * COM6 v126 - L3-Aware NC + 4x B-Pack
 * ======================================
 * v125 core + two targeted improvements for single-thread and pack throughput:
 *
 * 1. L3 cache detection + L3-aware NC auto-tuning:
 *    v125 uses NC=2048 for 1T unconditionally. With auto KC=640 on GETH,
 *    the B-panel is 640*2048*8 = 10MB. If L3 is 8-11MB, this overflows.
 *    v126 detects L3 size and caps NC so B-panel fits in 75% of L3.
 *    For JC-parallel, NC_JC is also capped (each thread has private B-panel).
 *
 * 2. 4x k-unrolled B-packing:
 *    A-pack already does 4x k-unroll with prefetch. B-pack was 2x.
 *    4x halves loop overhead and improves prefetch amortization.
 *
 * 3. 16384 in default benchmark on servers, CLI supports up to 32768.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v126 com6_v126.c -lm
 */
...
int main(int argc, char**argv){
    ...
    printf("COM6 v126 - L3-Aware NC + 4x B-Pack\n");
    ...
    printf("\nv126: L3-aware NC, 4x B-pack. Strassen depth=%d, KC=%d, NC_1T=%d, NC_JC=%d.\n",
    ...
    printf("Run individual: ./com6_v126 <size> [mt|1t]\n");
