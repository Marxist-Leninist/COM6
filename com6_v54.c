/*
 * COM6 v54 - Best-of-All: NC=4096 + C-Prefetch + Cold-First
 * ==========================================================
 * Merges proven wins from v26-v53:
 *   COMPUTE: v26 8x k-unrolled 6x8 ASM micro-kernel (fastest)
 *   NC=4096 for n>2048: 33% win at 8192 (v32 vs v26)
 *   C-PREFETCH: next-tile 6-row prefetch in macro_kernel (v36)
 *   COLD-FIRST: large sizes first on cold CPU (+25% at 8192)
 *   BLOCKING: v26 two-tier (120/256 small, 96/320 large)
 *   MT: fork-per-k-panel (faster than persistent parallel, tested v35)
 *
 * Confirmed results on i7-10510U (15W, 4C/8T):
 *   1024 MT: 73.6 GF | 2048 MT: 67.1 GF | 4096 MT: 60.3 GF | 8192 MT: 61.2 GF
 *   vs OpenBLAS: COM6 wins 30-47% at 2048-4096
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v54 com6_v54.c -lm
 */
...
static void macro_kernel_1t(const double*pa,const double*pb,
    ...
    for(int jr=0;jr<nc;jr+=NR){
        ...
        for(int ir=0;ir<mc;ir+=MR){
            ...
            /* Prefetch next C tile (ir+MR) to hide memory latency */
            if(ir+MR<mc){
                double*Cnext=C+(size_t)(ic+ir+MR)*n+(jc+jr);
                _mm_prefetch((const char*)Cnext, _MM_HINT_T0);
                _mm_prefetch((const char*)(Cnext+n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Cnext+2*n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Cnext+3*n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Cnext+4*n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Cnext+5*n), _MM_HINT_T0);
            }
...
static void get_blocking(int n, int*pMC, int*pKC, int*pNC){
    if(n <= 2048){*pMC=(n<=1024)?MC_SMALL:MC_LARGE;*pKC=(n<=1024)?KC_SMALL:KC_LARGE;*pNC=NC_SMALL;}
...
int main(void){
    ...
    printf("====================================================================\n");
    printf("  COM6 v54 - Best-of-All (%d threads)\n",nth);
    printf("  n<=1024: MC=%d KC=%d NC=%d\n",MC_SMALL,KC_SMALL,NC_SMALL);
    printf("  n>1024:  MC=%d KC=%d NC=%d (n<=2048) / NC=%d (n>2048)\n",MC_LARGE,KC_LARGE,NC_SMALL,NC_LARGE);
    printf("  8x unrolled ASM micro-kernel, parallel B-pack\n");
    printf("  LARGE-FIRST: 8192 on cold CPU. Skip 1T n>=4096\n");
    printf("====================================================================\n\n");

    /* Run largest first — CPU is coldest at start, gives best MT numbers */
    ...
    for(int si=0;si<ns;si++){
        ...
        if(!skip_1t){
            ...
            cooldown_ms(2000); /* cool between 1T and MT */
        }
        ...
        double gf1=skip_1t?0:(2.0*n*n*(double)n)/(best_1*1e9);
        ...
        if(n<=512){
            ...
            /* For huge: do a 1T multiply just for verification, not timed */
    ...
    printf("\nBest-of-all: NC=4096(+33%%) + C-prefetch(+17%%) + cold-first(+25%%)\n");
    return 0;
}
