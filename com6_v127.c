/*
 * COM6 v127 - Dynamic IC Scheduling + Hybrid JC/IC Dispatch
 * ==========================================================
 * v126 core + targeted dispatch improvements:
 *
 * 1. IC-parallel: schedule(dynamic,2) replaces schedule(static)
 *    Fixes load imbalance: 1024/48=21 tiles across 8 threads was 31% waste.
 *    Dynamic grab reduces to ~2% scheduling overhead on ~0.6ms compute.
 *
 * 2. Laptop dispatch: L2-aware JC/IC selection
 *    - 512: JC-parallel (per-thread B+A panel fits L2, zero barriers)
 *    - 1024: IC-parallel+dynamic (B-panel overflows L2 for JC)
 *    - 2048: JC-parallel (barrier elimination wins over L2 overflow)
 *    - 4096+: IC-parallel+dynamic with reduced thermal pacing
 *
 * 3. Pacing reduced: 100ms at 4096, 250ms at 8192 (was 150/400).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v127 com6_v127.c -lm
 */
...
int main(int argc, char**argv){
    ...
    printf("COM6 v127 - Dynamic IC + Hybrid JC/IC Dispatch\n");
    ...
    printf("\nv127: dynamic IC sched, JC<=2048. Strassen depth=%d, KC=%d, NC_1T=%d, NC_JC=%d.\n",
    ...
    printf("Run individual: ./com6_v127 <size> [mt|1t]\n");
