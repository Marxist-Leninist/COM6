/*
 * COM6 v48 - No-Pack Small Matrix + v48 Core
 * =============================================
 * v48 loses at 256 (38 GF vs BLAS 101 GF) because packing overhead dominates
 * at small sizes. At 256x256, the entire matrix is only 512KB — fits in L2!
 * Packing wastes time copying data that's already cache-resident.
 *
 * Fix: for n<=256, skip packing entirely. Instead:
 * - Use ikj loop order with AVX2 FMA directly on unpacked matrices
 * - B is accessed column-stride (bad) but 256*8 = 2KB rows fit L1
 * - OpenMP parallel on outer i-loop
 * - For n=256: 2*256^3 = 33.6M FLOPs, each thread handles 32 rows = 4.2M FLOPs
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v48 com6_v48.c -lm
 */
 ...
 * Matrices fit L2 (256x256*8 = 512KB), packing is pure overhead.
...
static void com6_multiply(const double*__restrict__ A,
                           ...
                           double*__restrict__ C,int n)
{
    /* Small-matrix fast path: no packing, direct AVX2 FMA */
    if(n <= 256){
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v48 - 4x Pack_A Unroll (%d threads)\n",nth);
    ...
    printf("\nv48: 4x unrolled pack_A for better ILP\n");
    return 0;
}
