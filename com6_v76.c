/*
 * COM6 v76 - Deep KC + C-Prefetch + Tuned Large Blocking
 * =======================================================
 * v75 base + optimizations for 8192+ sizes:
 *   - Deep KC=512 for n>4096: reduces C-traffic passes from 22→16 at 8192
 *     A-panel: 48*512*8=192KB (fits L2 with 64KB headroom)
 *   - NC=2048 for huge tier: reduces jc iterations from 6→4 (fewer B repacks)
 *     B-panel: 2048*512*8=8MB (fills L3 exactly — max reuse across ic tiles)
 *   - MC=48 for huge MT: 8192/48=171 ic tiles → excellent thread distribution
 *   - C-prefetch in beta1 kernel: prefetch C rows before load+add phase
 *   - All v75 features: NT stores, beta=0 memset elimination, 3-tier blocking
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v76 com6_v76.c -lm
 */
...
/* Deep KC for 8192+: fewer passes through C = less DRAM traffic
 * KC=512: 8192/512=16 passes (was 22 with KC=384) → 27% less C traffic
 * A-panel: 48*512*8=192KB (fits 256KB L2)
 * B-panel: 2048*512*8=8MB (fills 8MB L3 — max reuse) */
#define KC_HUGE  512
#define MC_HUGE  48
#define NC_HUGE  2048
...
/* ================================================================
 * MICRO-KERNEL 6x8 - 8x k-unrolled inline ASM
 * ================================================================
 * 8 rank-1 updates per loop iteration.
 * Per iteration: 8*12=96 FMAs, loop overhead: 3 instructions.
 * Ratio: 96/(96+3) = 97% useful work (vs 94% with 4x unroll).
 *
 * A stride per k: MR*8 = 48 bytes
 * B stride per k: NR*8 = 64 bytes
 * 8 k-steps: A advances 384 bytes, B advances 512 bytes.
 */
                ...
                double* C, int ldc)
{
    ...
    __asm__ volatile(
        /* Prefetch C rows NOW — k-loop gives 192+ cycles to hide DRAM latency */
        "movq %[C],%%r15\n\t"
        "prefetcht0 (%%r15)\n\t"   "prefetcht0 32(%%r15)\n\t"
        "addq %[ldc],%%r15\n\t"
        "prefetcht0 (%%r15)\n\t"   "prefetcht0 32(%%r15)\n\t"
        ...
        /* Prefetch: A+768 bytes ahead (~16 k-steps), B+1024 bytes ahead (~16 k-steps) */
        "prefetcht0 768(%[pA])\n\t"
        "prefetcht0 1024(%[pB])\n\t"
        ...
        /* Advance: A += 8*MR*8 = 384, B += 8*NR*8 = 512 */
        "addq $384,%[pA]\n\t"
        "addq $512,%[pB]\n\t"
        "decq %[kc8]\n\t"
        "jnz 1b\n\t"

        /* Remainder loop (1 at a time) */
        "3:\n\t"
        "testq %[kcr],%[kcr]\n\t"
        "jle 2f\n\t"
        "4:\n\t"
        ...
        /* Store back: C += accumulators (C already in L1 from prefetch at kernel start) */
        "2:\n\t"
        "vaddpd (%[C]),%%ymm0,%%ymm0\n\t"  "vmovupd %%ymm0,(%[C])\n\t"
...
/* beta=0 variant: just store (no load from C, no memset needed) */
static void __attribute__((noinline))
                ...
                double* C, int ldc)
{
    ...
    __asm__ volatile(
        ...
        /* Store: just store (beta=0, no load from C) */
        "2:\n\t"
        "vmovupd %%ymm0,(%[C])\n\t"
        "vmovupd %%ymm1,32(%[C])\n\t"
        "addq %[ldc],%[C]\n\t"
                   ...
                   double* C, int ldc)
{
    ...
    __asm__ volatile(
        ...
        /* Store: NT store (beta=0, bypass cache) */
        "2:\n\t"
        "vmovntpd %%ymm0,(%[C])\n\t"
        "vmovntpd %%ymm1,32(%[C])\n\t"
        "addq %[ldc],%[C]\n\t"
...
/* 1T blocking: maximize L2 utilization (MC fills L2) */
...
/* MT blocking: smaller MC for n<=1024 gives more ic tiles = better thread distribution */
#define MC_MT_SMALL 48  /* 512/48=11 tiles (vs 512/120=5), 1024/48=22 (vs 1024/120=9) */
    ...
    for(int jc=0;jc<n;jc+=nc_blk){int nc=(jc+nc_blk<=n)?nc_blk:n-jc;
        for(int pc=0;pc<n;pc+=kc_blk){int kc=(pc+kc_blk<=n)?kc_blk:n-pc;
            ...
            for(int ic=0;ic<n;ic+=mc_blk){int mc=(ic+mc_blk<=n)?mc_blk:n-ic;
                ...
                macro_kernel_1t(pa,pb,C,mc,nc,kc,n,ic,jc,beta,0);
    }}}
    af(pa);af(pb);
}
                           ...
                           double*__restrict__ C,int n)
{
    ...
    int use_nt = (n >= 2048);  /* NT stores for large C matrices */
    ...
    if(n <= 2048){
        /* Per-tile parallel region (small/medium) */
        ...
        /* Single parallel region for 4096+ (no fork/join between tiles) */
        #pragma omp parallel
        {
            int tid = omp_get_thread_num();
            int nt = omp_get_num_threads();
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v76 - Deep KC + C-Prefetch + Tuned Large (%d threads)\n",nth);
    printf("  n<=1024: MC=%d KC=%d NC=%d\n",MC_SMALL,KC_SMALL,NC_DEFAULT);
    printf("  n<=4096: MC=%d KC=%d NC=%d\n",MC_LARGE,KC_LARGE,NC_DEFAULT);
    printf("  n>=8192: MC=%d KC=%d NC=%d (B-panel 8MB, fills L3)\n",MC_HUGE,KC_HUGE,NC_HUGE);
    printf("  C-prefetch, deep-KC (16 passes vs 22), 4x A-pack\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv76: Deep KC=512 for 8192 + C-prefetch + NT stores + memset elimination.\n");
    printf("Run individual: ./com6_v76 <size> [mt|1t]\n");
    return 0;
}
