/*
 * COM6 v60 - 4x A-Pack Unroll + Parallel C-zeroing
 * =================================================
 * v58 + two improvements:
 * 1. 4x k-unrolled A-packing: 24 loads + 24 stores per iteration,
 *    halving loop overhead vs 2x unroll. KC=256 (divisible by 4) and
 *    KC=320 (divisible by 4) both work cleanly.
 * 2. Parallel memset for C buffer (distribute page faults).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v60 com6_v60.c -lm
 */
...
static void pack_A(const double*__restrict__ A,double*__restrict__ pa,
    ...
    for(int i=0;i<mc;i+=MR){
        ...
        if(mr==MR){
            ...
            int k=0;
            /* 4x k-unrolled: 24 loads + 24 stores per iteration */
            for(;k+3<kc;k+=4){
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* Parallel memset for large matrices */
    if(use_mt && n >= 2048){
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v60 - Streaming B-Pack + Dynamic Schedule (%d threads)\n",nth);
