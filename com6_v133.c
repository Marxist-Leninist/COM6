/*
 * COM6 v133 - Strassen memset elimination + C-prefetch in beta0 kernel
 * =====================================================================
 * CHANGE 1: Eliminate memset(C,0) in Strassen.
 *   First accumulation per C-quadrant uses c_set (copy), rest add/sub.
 *   Saves one full C-matrix of wasted write traffic (512MB at 8192).
 *
 * CHANGE 2: Add C-row prefetch in beta0 kernel for better store bandwidth.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v133 com6_v133.c -lm
 */
...
micro_6x8_beta0(int kc, const double* pA, const double* pB, double* C, int ldc)
{
    ...
    __asm__ volatile(
        /* Prefetch C rows early — hides write-allocate latency at store time */
        "movq %[C],%%r15\n\t"
        "prefetcht0 (%%r15)\n\t"
        "addq %[ldc],%%r15\n\t"
        "prefetcht0 (%%r15)\n\t"
...
int main(int argc, char**argv){
    ...
    printf("COM6 v133 - Persistent Pool + NT Stores for Large Matrices\n");
    ...
    printf("\nv133: v130 + non-temporal stores (vmovntpd) for n>=4096.\n");
    ...
    printf("Run individual: ./com6_v133 <size> [mt|1t]\n");
