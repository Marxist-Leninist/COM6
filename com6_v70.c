/*
 * COM6 v70 - Adaptive NT Store Threshold
 * =======================================
 * v68 base + smart NT store control:
 *   Problem: NT stores (vmovntpd) bypass cache. For small matrices where
 *   C fits in L3 (n<=1024 -> C<=8MB), NT stores on beta=0 evict C from
 *   cache, causing misses on subsequent beta=1 KC-tile reads.
 *   This killed 512 1T (28.5 GF vs 49.1 in v24) and 1024 1T (21.2 GF).
 *
 *   Fix: NT stores only when C > L3 (n>=2048, C=32MB).
 *   For n<2048: beta=0 uses regular vmovupd (keeps C in cache).
 *   For n>=2048: beta=0 uses vmovntpd (avoids polluting L3 with huge C).
 *
 *   Also: for n<2048 1T, use memset+always-beta=1 path (simpler, C stays hot).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v70 com6_v70.c -lm
 */
...
static void get_blocking_mt(int n, int*pMC, int*pKC, int*pNC){
    ...
    else if(n <= 1024){*pMC=MC_HUGE; *pKC=KC_SMALL;*pNC=NC_DEFAULT;} /* MC=72: balanced */
    else if(n <= 4096){*pMC=MC_LARGE;*pKC=KC_LARGE;*pNC=NC_DEFAULT;}
    else              {*pMC=MC_HUGE; *pKC=KC_HUGE;  *pNC=NC_HUGE;}
}
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v70 - Adaptive NC + v67 Core (%d threads)\n",nth);
    printf("  n<=4096: NC=%d | n>4096: NC=%d KC=%d MC=%d (B=4.72MB, L3-fit)\n",NC_DEFAULT,NC_HUGE,KC_HUGE,MC_HUGE);
    ...
    printf("NC=%d for n>4096, KC=%d/MC=%d for huge sizes\n",NC_HUGE,KC_HUGE,MC_HUGE);
    printf("Run individual sizes: ./com6_v70 <size> [mt|1t]\n");
    return 0;
}
