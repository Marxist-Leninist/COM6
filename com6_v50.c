/*
 * COM6 v50 - Unified NC + Distributed Prefetch + Dynamic Scheduling
 * ==================================================================
 * Changes from v49:
 * 1. NC=2048 for ALL sizes (was NC=1024 for n>4096 — too conservative)
 *    B-panel: 2048*320*8=5MB fits 8MB L3. Halves jc iterations for 8192,
 *    cutting A-panel repacking overhead in half.
 * 2. schedule(dynamic,2) for ic-loop — better load balance than static,
 *    less scheduling overhead than dynamic,1.
 * 3. Distributed prefetch: pA and pB prefetch spread across k+0,k+2,k+4,k+6
 *    instead of bunched at k+0 and k+4. Keeps memory pipeline fed.
 * 4. Pack-A with source-row prefetch for next panel.
 *
 * Compile:
...
static void get_blocking(int n, int use_mt, int*pMC, int*pKC, int*pNC){
    ...
    else{*pMC=MC_LARGE;*pKC=KC_LARGE;*pNC=NC_HUGE;}
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v50 - Unified NC + Distributed Prefetch (%d threads)\n",nth);
    printf("  Blocking: <=1024: MC=%d KC=%d NC=%d\n",MC_SMALL,KC_SMALL,NC_STD);
    printf("            <=4096: MC=%d KC=%d NC=%d\n",MC_LARGE,KC_LARGE,NC_STD);
    printf("            >4096:  MC=%d KC=%d NC=%d\n",MC_LARGE,KC_LARGE,NC_HUGE);
    printf("  schedule(dynamic,2) + distributed prefetch\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv50: distributed prefetch + pack-A prefetch + tiered NC\n");
    return 0;
}
