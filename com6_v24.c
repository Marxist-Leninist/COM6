/*
 * COM6 v24 - 8x Unrolled ASM + Streaming C Stores + Tuned Prefetch
 * =================================================================
 * Changes from v23:
 * 1. 8x k-unrolled micro-kernel (was 4x) — halves loop overhead again
 * 2. Prefetch next A/B panels deeper (prefetcht0 for L1, prefetcht1 for L2)
 * 3. Interleaved A-broadcast pattern: load 2 B vectors, then alternate
 *    broadcasts to maximize FMA port utilization on dual-FMA CPUs
 * 4. Try KC=512 for better amortization of packing (fits L2 with MC=72)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v24 com6_v24.c -lm
 */
...
int main(void){
    ...
    printf("====================================================================\n");
    printf("  COM6 v24 - 8x Unrolled ASM + Parallel B-Pack (%d threads)\n",nth);
    printf("  MC=%d KC=%d NC=%d  8x unrolled ASM micro-kernel\n",MC,KC,NC);
    ...
    printf("\n8x k-unrolling: 97%% useful FMA work per loop iteration (vs 94%% with 4x)\n");
    return 0;
}
