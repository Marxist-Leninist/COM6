#define _GNU_SOURCE
/*
 * COM6 v119 - Pre-allocated persistent pool + restored IC-parallel for small sizes
 * ================================================================================
 * v118 regressed on monolithic at 512/1024 by routing all n<4096 to JC-parallel.
 * JC-parallel has redundant A-packing (all threads pack the same A rows).
 * v108 (laptop champion) correctly used IC-parallel (shared B, work-stealing IC)
 * for 512/1024 — much less total packing work at small sizes.
 *
 * v119 combines:
 *   - v118's persistent pthreads pool (eliminates OpenMP ~100μs fork-join overhead)
 *   - v108's dispatch for monolithic: IC-par for 512/1024, JC-par for 2048,
 *     IC-par for 4096+ (proven best on i7-10510U)
 *   - Pre-allocated per-thread buffers in pool struct (zero malloc per multiply)
 *   - v118's chiplet detection + JC-parallel for chiplet CPUs
 *   - 2x B-pack unroll from v77 (proven throughput win)
 *   - v108's thermal pacing at n>=4096
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -lpthread \
 *       -o com6_v119 com6_v119.c -lm
 */
...
/* ================================================================
 * PERSISTENT THREAD POOL with pre-allocated buffers
 * ================================================================ */
...
typedef struct {
    ...
    /* Pre-allocated per-thread buffers (allocated once at pool_init) */
    double* pa_bufs[MAX_THREADS];   /* per-thread A-panel */
    double* pb_bufs[MAX_THREADS];   /* per-thread B-panel (JC path) */
    double* pb_shared;              /* shared B-panel (IC path) */
} thread_pool_t;
...
static void pool_init(void) {
    ...
    for (int t = 0; t < ncpus; t++) {
        g_pool.pa_bufs[t] = aa((size_t)MC_MAX * KC_MAX);
        g_pool.pb_bufs[t] = aa((size_t)KC_MAX * NC_MAX);
        /* Pre-touch to avoid page faults on first use */
        memset(g_pool.pa_bufs[t], 0, (size_t)MC_MAX * KC_MAX * sizeof(double));
        memset(g_pool.pb_bufs[t], 0, (size_t)KC_MAX * NC_MAX * sizeof(double));
    }
    g_pool.pb_shared = aa((size_t)KC_MAX * NC);
    memset(g_pool.pb_shared, 0, (size_t)KC_MAX * NC * sizeof(double));
...
static void pool_shutdown(void) {
    ...
    for (int t = 0; t < g_pool.thread_count; t++) {
        af(g_pool.pa_bufs[t]); g_pool.pa_bufs[t] = NULL;
        af(g_pool.pb_bufs[t]); g_pool.pb_bufs[t] = NULL;
    }
    af(g_pool.pb_shared); g_pool.pb_shared = NULL;
...
/* ================================================================
 * PACKING (2x k-unrolled B-pack from v77)
 * ================================================================ */
...
/* ================================================================
 * IC-PARALLEL (persistent pool, shared B, work-stealing IC)
 * ================================================================
 * Uses pre-allocated pa_bufs from pool + shared pb_shared.
 * Zero malloc per multiply call. Cooperative B-pack + spin barrier
 * + atomic IC work-stealing. ~1μs dispatch overhead.
 * ================================================================ */
...
static void ic_worker(int tid, int nthreads, void* arg) {
    ...
    double* pa = g_pool.pa_bufs[tid];
    double* pb = g_pool.pb_shared;
    int n = w->n;
    int mc_blk = w->mc_blk;
    int kc_blk = w->kc_blk;
    int phase = 0;
    ...
    for (int jc = 0; jc < n; jc += NC) {
        ...
        for (int pc = 0; pc < n; pc += kc_blk) {
            ...
            /* Cooperative B-pack: each thread packs its chunk */
            ...
            if (j_start < nc)
                pack_B_chunk(w->B, pb, kc, nc, n, jc, pc, j_start, j_end);
            ...
            /* Atomic work-stealing for IC blocks */
            int ic;
...
/* ================================================================
 * JC-PARALLEL (persistent pool, private B-panels, pre-allocated bufs)
 * ================================================================
 * Each thread owns a column slab. Zero barriers, zero C contention.
 * Uses pre-allocated pa_bufs + pb_bufs from pool (zero malloc per call).
 * Best for chiplet CPUs (no cross-CCD traffic) and monolithic at 2048.
 * ================================================================ */
typedef struct {
    const double* A;
    const double* B;
...
/* ================================================================
 * DISPATCH — v108-proven routing for monolithic, v118 for chiplet
 * ================================================================
 * Monolithic (laptop):
 *   512/1024:  IC-parallel (shared B, work-stealing — less total A-packing)
 *   2048:      JC-parallel (each thread owns columns — proven best warm-CPU)
 *   4096+:     IC-parallel (shared B + thermal pacing)
 * Chiplet (EPYC/Ryzen):
 *   All sizes: JC-parallel (private B panels, zero cross-CCD traffic)
 * ================================================================ */
static void com6_multiply(const double*__restrict__ A,
                           const double*__restrict__ B,
                           double*__restrict__ C, int n)
{
    ...
    if (g_is_chiplet) {
        /* Chiplet: always JC-parallel (zero cross-CCD traffic) */
        int mc_blk, kc_blk;
        get_blocking_mt(n, &mc_blk, &kc_blk);
        com6_multiply_jc(A, B, C, n, mc_blk, kc_blk);
    } else {
        /* Monolithic: v108 proven dispatch */
        if (n >= 4096) {
            com6_multiply_ic(A, B, C, n);
        } else if (n >= 2048) {
            int mc_blk, kc_blk;
            ...
            /* 512/1024: IC-parallel — shared B + work-stealing is more
             * efficient than JC's redundant A-packing at small sizes */
            com6_multiply_ic(A, B, C, n);
        }
    }
}
...
int main(int argc, char**argv){
    ...
    printf("COM6 v119 - persistent pool + v108 dispatch + pre-alloc buffers\n");
    ...
    printf("Dispatch: %s\n\n", g_is_chiplet ?
           ...
           "512/1024 IC-par | 2048 JC-par | 4096+ IC-par+pace");
    ...
    printf("\nv119: persistent pool (no OpenMP fork-join) + v108 dispatch + pre-alloc bufs.\n");
    ...
    printf("Run individual: ./com6_v119 <size> [mt|1t]\n");
