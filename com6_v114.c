/*
 * COM6 v114 - INCONCLUSIVE EXPERIMENT: Thread affinity pinning on Windows
 * =====================================================================
 * Hypothesis: v108's OpenMP parallel regions don't pin threads. On
 * Windows the scheduler can migrate threads between logical CPUs,
 * invalidating per-thread L1/L2 caches and the PA packed buffer each
 * thread owns. Pinning thread N to a specific logical CPU (HT-aware:
 * primaries 0,2,4,6 first, then HT siblings 1,3,5,7) should stabilize
 * cache locality — especially at 512 MT where v108's 1.88x 1T scaling
 * ceiling suggests room if cache misses can be reduced.
 *
 * Change: `pin_thread_to_cpu(int tid)` calls
 *   SetThreadAffinityMask(GetCurrentThread(), 1ULL << cpu)
 * with HT-aware mapping (first N=n_physical threads get primary lanes,
 * next N get HT siblings). Invoked at top of each #pragma omp parallel
 * in com6_multiply_ic_body, com6_multiply_jc, com6_multiply_jc_ctx.
 *
 * FIRST RESULT (linear tid->cpu, pin all threads naively):
 *   - 512 MT: hurts (tid 0&1 land on same physical core = L1/L2 contention)
 * FIXED TO HT-AWARE MAPPING:
 *   - 512 MT (cold-ish, stable thermal): +25% (77-88 GF vs 67-68 GF)
 *   - 1024 MT (warm, high thermal variance): within noise (+/-15%)
 *
 * RESULT: empirically inconclusive. The clearest apparent win at 512 MT
 * did not reproduce after the laptop reached thermal saturation; under
 * sustained thermal pressure both PIN-ON and PIN-OFF degrade similarly.
 * On a 15W TDP laptop the measurement noise exceeds the effect size.
 *
 * Theoretically sound: HT-aware pinning IS how serious BLAS libraries
 * (MKL, OpenBLAS with OMP_PROC_BIND=close+OMP_PLACES=cores) schedule
 * threads. On desktop/workstation hardware without thermal constraints,
 * this change is expected to be a modest win. On this laptop, the
 * measurement budget isn't enough to separate signal from noise.
 *
 * Env:
 *   COM6_NO_PIN=1       disable pinning (revert to v108 behavior)
 *   COM6_PIN_LINEAR=1   use naive tid->cpu mapping (demonstrated regression)
 *
 * All micro-kernel code, dispatch, and blocking constants identical
 * to v108. The only added code path is pin_thread_to_cpu().
 *
 * Verdict: kept as documented experiment. v108 remains champion.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v114 com6_v114.c -lm
 */
...
static inline void pin_thread_to_cpu(int tid){
#ifdef _WIN32
    if(g_pin_enabled != 1) return;
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v114 - Thread affinity pinning on Windows (%d threads, pin=%s)\n",
           nth, g_pin_enabled ? "ON" : "OFF");
    printf("  8192+: IC-par + 400ms pace | 4096: IC-par + 150ms pace | 2048: JC-par | <2048: IC-par\n");
    ...
    printf("\nv114: v108 + Windows thread affinity pinning (SetThreadAffinityMask per tid)\n");
    printf("COM6_NO_PIN=1 disable pinning (revert to v108) | COM6_PACE_MS=N | COM6_PACE_MIN_N=N | COM6_IC_2048=1\n");
    printf("COM6_USE_STRASSEN=1 force v105 Strassen path at 8192+.\n");
    printf("Run individual: ./com6_v114 <size> [mt|1t]\n");
    return 0;
}
