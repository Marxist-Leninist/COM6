/*
 * COM6 v40 - Single Parallel Region + C-Prefetch Micro-Kernel
 * =============================================================
 * v30-v39 all regressed from v26. This version keeps v26's proven core
 * and adds two targeted optimizations:
 *
 * 1) Single parallel region: instead of entering/exiting the OpenMP
 *    parallel region for every pc iteration, we wrap the entire
 *    jc+pc loop in ONE parallel region with explicit barriers.
 *    Saves ~50us per pc iteration on fork/join overhead.
 *
 * 2) C-prefetching micro-kernel: prefetches the C output rows at the
 *    START of the micro-kernel so the writeback loads (vaddpd from C)
 *    don't stall. Critical for 8192 where C (512MB) is completely
 *    out of cache.
 *
 * 3) 8192 support: NC=1024 for n>=4096 (reduces B-panel to 2MB,
 *    leaves room in L3 for C tiles and A panels).
 *
 * Blocking (v26 proven + 8192 tier):
 *   n<=1024: MC=120 KC=256 NC=2048
 *   n<=4096: MC=96  KC=320 NC=2048
 *   n>=4096: MC=96  KC=320 NC=1024 (halve L3 pressure)
 *
 * Compile:
...
#define MC_LARGE 96
#define NC_MED   2048
#define NC_LARGE 1024    /* Halve NC for 8192+ to reduce L3 pressure */
...
/*
 * 6x8 micro-kernel with C-prefetching.
 * Prefetches C output rows at the top so the writeback vaddpd loads
 * don't stall on cache misses (critical for 8192 where C >> L3).
 */
...
micro_6x8(int kc, const double* pA, const double* pB, double* C, int ldc)
{
    ...
    __asm__ volatile(
        /* Prefetch C output rows BEFORE computation */
        "prefetcht0 (%[C])\n\t"
        "prefetcht0 32(%[C])\n\t"
        "movq %[C],%%r15\n\t"
        "addq %[ldc],%%r15\n\t"
...
static void get_blocking(int n, int*pMC, int*pKC, int*pNC){
    ...
    else{*pMC=MC_LARGE;*pKC=KC_LARGE;*pNC=NC_LARGE;}  /* 8192+: NC=1024 less L3 pressure */
}
...
int main(void){
    ...
    for(int si=0;si<ns;si++){
        ...
        if(n<=512){
            ...
            /* For huge sizes, verify sub-block */
    ...
    printf("\nv40: Single parallel region + C-prefetch + NC=1024 for >2048\n");
    return 0;
}
