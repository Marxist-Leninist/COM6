/*
 * COM6 v92 - Dynamic Scheduling + 2x B-Pack
 * ==========================================
 * Builds on v91 with:
 * 1. schedule(dynamic,2) for n>=2048: thermal throttling on 15W laptop
 *    causes uneven thread speeds. Dynamic dispatch lets fast threads
 *    steal work from slow (throttled) threads. Chunk=2 balances
 *    dispatch overhead vs load balance.
 * 2. 2x k-unrolled B-packing: process 2 rows per iteration in NR=8
 *    panels. Halves loop overhead. From v77.
 * 3. Reverse benchmark order: 8192 first when CPU is coldest.
 * 4. All v91 features: beta-0/1 kernels, C-prefetch, CLI mode.
 *
 * Blocking (from v26, proven optimal on i7-10510U):
 *   n<=1024: MC=120 KC=256 NC=2048  (120*256*8=240KB, fits L2)
 *   n>1024:  MC=96  KC=320 NC=2048  (96*320*8=240KB, fits L2)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v92 com6_v92.c -lm
 */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* v26's proven MT approach: fork-join per (jc,pc) panel */
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v92 - Dynamic Sched + 2x B-Pack (%d threads)\n",nth);
    printf("  1T: MC=%d/%d KC=%d/%d | MT: MC=%d/%d KC=%d/%d | NC=%d\n",
           MC_SMALL,MC_LARGE,KC_SMALL,KC_LARGE,MC_MT_SMALL,MC_LARGE,KC_SMALL,KC_LARGE,NC);
    printf("  Beta-0/1, C-prefetch, 2x B-pack, dynamic(2) for n>=2048\n");
    ...
    printf("\nv92: v91 + dynamic scheduling + 2x B-pack unroll\n");
    printf("Run individual: ./com6_v92 <size> [mt|1t]\n");
    return 0;
}
