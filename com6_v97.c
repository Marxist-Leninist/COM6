/*
 * COM6 v97 - IC-Parallel MC Tuning for Thread Balance
 * =====================================================
 * Changes from v96:
 * - IC-parallel uses MC=48 for ALL sizes (not just n<=1024)
 *   48*320*8=120KB (fits L2), 48/6=8 micro-tiles (perfect for MR=6)
 *   At n=4096: 86 ic-iterations / 8 threads = 10-11 per thread (vs v96's 5-6)
 * - Better thread load balancing = less idle time at barriers
 * - Saves 12.5% of FLOPs (137G FLOPS at n=8192)
 * - Submatrix additions are O(n^2) — negligible overhead
 * - Each sub-multiply uses full JC-parallel BLIS GEMM
 *
 * For n<8192: identical to v94 (JC-parallel for n>=2048, IC for n<2048)
 *
 ...
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v97 com6_v97.c -lm
 */
...
static void get_blocking_mt(int n, int*pMC, int*pKC){
    /* MC=48 for all IC-parallel: better thread balance + fits L2 easily
     * 48*256*8=96KB or 48*320*8=120KB — both well under L2=256KB */
    if(n <= 1024){*pMC=MC_MT_SMALL;*pKC=KC_SMALL;}
    else{*pMC=MC_MT_SMALL;*pKC=KC_LARGE;}
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v97 - IC-Parallel MC Tuning (%d threads)\n",nth);
    ...
    printf("\nv97: MC=48 all IC-par, Strassen@8192, JC-par@2048\n");
    printf("Saves 12.5%% FLOPs at 8192, each sub-mul uses full BLIS GEMM\n");
    printf("Run individual: ./com6_v97 <size> [mt|1t]\n");
    return 0;
}
