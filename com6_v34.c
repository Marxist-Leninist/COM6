/*
 * COM6 v34 - JC-Parallel Small-Matrix Dispatch
 * ================================================
 * Key changes from v33:
 * - JC-loop parallelization for small matrices on many-core systems
 *   (splits columns across thread groups — each group packs/computes independently)
 * - When ic_blocks < nthreads, auto-splits into jc-groups to keep all cores busy
 * - Large matrices unchanged (single-group fast path)
 *
 * Compile (AVX-512):
 *   gcc -O3 -march=native -mfma -funroll-loops -o com6_v34 com6_v34.c -lm -lpthread
 *
 * Compile (AVX2 only):
 *   gcc -O3 -march=native -mavx2 -mfma -mno-avx512f -funroll-loops -o com6_v34 com6_v34.c -lm -lpthread
 */
...
/* JC-parallel config: split threads into groups for small matrices */
typedef struct {
    int num_groups;         /* 1 = current behavior, 2+ = jc-parallel */
    int threads_per_group;
} jc_config_t;

static jc_config_t get_jc_config(int n, int nthreads){
    jc_config_t cfg;
    int mc_blk, kc_blk, nc_blk;
    get_blocking(n, &mc_blk, &kc_blk, &nc_blk);
    int ic_blocks = (n + mc_blk - 1) / mc_blk;
    /* Only engage jc-parallel on many-core with insufficient ic-blocks */
    if(nthreads <= 8 || ic_blocks >= nthreads){
        cfg.num_groups = 1;
        cfg.threads_per_group = nthreads;
        return cfg;
    }
    /* Pick groups so each has enough ic work */
    int desired = (nthreads + ic_blocks - 1) / ic_blocks;
    int max_by_cols = n / NR;  /* at least NR cols per group */
    if(desired > max_by_cols) desired = max_by_cols;
    if(desired < 2) desired = 2;
    cfg.num_groups = desired;
    cfg.threads_per_group = nthreads / desired;
    return cfg;
}
...
/* ================================================================
 * MERGED DISPATCH + JC-PARALLEL
 * ================================================================ */

#define MAX_JC_GROUPS 16

typedef struct {
    const double* A;
    const double* B;
    double* C;
    double** pb_bufs;       /* one per jc-group */
    double** pa_bufs;
    int n, nc, kc, mc_blk, jc, pc;
    int num_groups, threads_per_group;
    int group_nc[MAX_JC_GROUPS];   /* nc for each group */
    int group_j0[MAX_JC_GROUPS];   /* column offset for each group */
    atomic_int next_ic[MAX_JC_GROUPS];
    spin_barrier_t barriers[MAX_JC_GROUPS];
} merged_work_t;
...
static void merged_worker(int tid, int nthreads, void* arg){
    ...
    if(w->num_groups == 1){
        /* === FAST PATH: identical to v33 for large matrices === */
        ...
        if(je > w->nc) je = w->nc;
        if(js < w->nc)
            pack_B_chunk(w->B, w->pb_bufs[0], w->kc, w->nc, w->n, w->jc, w->pc, js, je);
        spin_barrier_wait(&w->barriers[0]);
        double* pa = w->pa_bufs[tid];
        int mc_blk = w->mc_blk;
        while(1){
            int ic = atomic_fetch_add_explicit(&w->next_ic[0], mc_blk, memory_order_relaxed);
            if(ic >= w->n) break;
            int mc = (ic + mc_blk <= w->n) ? mc_blk : w->n - ic;
            pack_A(w->A, pa, mc, w->kc, w->n, ic, w->pc);
            macro_kernel(pa, w->pb_bufs[0], w->C, mc, w->nc, w->kc, w->n, ic, w->jc);
        }
        return;
    }

    /* === JC-PARALLEL PATH: split threads into groups by column range === */
    int grp = tid / w->threads_per_group;
    int local_tid = tid % w->threads_per_group;
    int local_nt = w->threads_per_group;
    /* Last group absorbs remainder threads */
    if(grp >= w->num_groups){
        grp = w->num_groups - 1;
        local_tid = tid - grp * w->threads_per_group;
        local_nt = nthreads - grp * w->threads_per_group;
    }

    int gnc = w->group_nc[grp];
    int gj0 = w->group_j0[grp];

    /* Pack this group's B-chunk */
    int npanels = (gnc + NR - 1) / NR;
    int pp = (npanels + local_nt - 1) / local_nt;
    int p0 = local_tid * pp, p1 = p0 + pp;
    if(p1 > npanels) p1 = npanels;
    int js = p0 * NR, je = p1 * NR;
    if(je > gnc) je = gnc;
    if(js < gnc)
        pack_B_chunk(w->B, w->pb_bufs[grp], w->kc, gnc, w->n, w->jc + gj0, w->pc, js, je);

    /* Group-local barrier */
    spin_barrier_wait(&w->barriers[grp]);

    /* IC work-stealing within group */
    double* pa = w->pa_bufs[tid];
    int mc_blk = w->mc_blk;
    while(1){
        int ic = atomic_fetch_add_explicit(&w->next_ic[grp], mc_blk, memory_order_relaxed);
        if(ic >= w->n) break;
        int mc = (ic + mc_blk <= w->n) ? mc_blk : w->n - ic;
        pack_A(w->A, pa, mc, w->kc, w->n, ic, w->pc);
        macro_kernel(pa, w->pb_bufs[grp], w->C, mc, gnc, w->kc, w->n, ic, w->jc + gj0);
    }
}
                               ...
                               double*__restrict__ C, int n)
{
    ...
    /* JC-parallel config */
    jc_config_t jcfg = get_jc_config(n, active_threads);
    int ngrp = jcfg.num_groups;

    double** pa_bufs = (double**)malloc(max_threads * sizeof(double*));
    for(int t = 0; t < max_threads; t++) pa_bufs[t] = aa((size_t)MC_MAX * KC_MAX);
    /* One pb buffer per jc-group */
    double** pb_bufs = (double**)malloc(ngrp * sizeof(double*));
    for(int g = 0; g < ngrp; g++) pb_bufs[g] = aa((size_t)KC_MAX * NC_MAX);

    memset(C, 0, (size_t)n * n * sizeof(double));

    /* Thermal: only for TDP-limited laptops */
    double baseline_rate = 0;
    int iter_count = 0, thermal_adjustments = 0;
    int min_thermal_threads = (active_threads + 1) / 2;
    int enable_thermal = (n >= 4096 && max_threads <= 8);

    merged_work_t mw;
    mw.A = A; mw.B = B; mw.C = C; mw.pb_bufs = pb_bufs; mw.pa_bufs = pa_bufs;
    mw.num_groups = ngrp; mw.threads_per_group = jcfg.threads_per_group;
    pool_set_active(active_threads);

    for(int jc = 0; jc < n; jc += nc_blk){
        int nc = (jc + nc_blk <= n) ? nc_blk : n - jc;

        /* Compute group column splits */
        if(ngrp > 1){
            int cols_per = ((nc / ngrp + NR - 1) / NR) * NR;
            int off = 0;
            for(int g = 0; g < ngrp; g++){
                mw.group_j0[g] = off;
                int rem = nc - off;
                mw.group_nc[g] = (g == ngrp - 1) ? rem : (cols_per < rem ? cols_per : rem);
                off += mw.group_nc[g];
            }
        }
        ...
        for(int pc = 0; pc < n; pc += kc_blk){
            ...
            /* Init per-group barriers and atomics */
            for(int g = 0; g < ngrp; g++){
                int grp_threads;
                if(ngrp == 1) grp_threads = active_threads;
                else if(g < ngrp - 1) grp_threads = jcfg.threads_per_group;
                else grp_threads = active_threads - g * jcfg.threads_per_group;
                spin_barrier_init(&mw.barriers[g], grp_threads);
                atomic_store(&mw.next_ic[g], 0);
            }
    ...
    free(pa_bufs);
    for(int g = 0; g < ngrp; g++) af(pb_bufs[g]);
    free(pb_bufs);
}
...
int main(void){
    ...
    pool_destroy();
    printf("\nv34: %s | NR=%d | auto %d threads | jc-parallel dispatch\n", KERNEL_NAME, NR, nthreads);
    return 0;
}
