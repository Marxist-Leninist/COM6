/*
 * COM6 v59 - Parallel memset + A-pack prefetch
 * =============================================
 * v58 + two improvements:
 * 1. Parallel memset: page-fault C buffer across threads (NUMA-aware on
 *    multi-socket, and avoids serial page-fault bottleneck at 8192 where
 *    C alone is 512MB).
 * 2. Pack-A source prefetch: prefetch next row of A while packing current,
 *    so data arrives in L1 before the scalar gather starts.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v59 com6_v59.c -lm
 */
...
#define NC_HUGE    1536   /* For n>4096: 3.84MB B-panel, 72% L3 */
#define NC_MAX     2048
#define ALIGN 64
...
static void pack_B_chunk(const double*__restrict__ B,double*__restrict__ pb,
    ...
    for(int j=j_start;j<j_end;j+=NR){
        ...
        if(nr==NR){for(int k=0;k<kc;k++){
            /* Streaming stores: bypass L1/L2 cache, write direct to L3 */
            _mm256_stream_pd(dest,_mm256_loadu_pd(Bkj));
            _mm256_stream_pd(dest+4,_mm256_loadu_pd(Bkj+4));
            dest+=NR;Bkj+=n;
...
static void pack_A(const double*__restrict__ A,double*__restrict__ pa,
    ...
    for(int i=0;i<mc;i+=MR){
        ...
        if(mr==MR){
            ...
            int k=0;
            for(;k+1<kc;k+=2){
                /* Prefetch ahead by 8 doubles (1 cache line) */
                if((k&6)==0){
                    _mm_prefetch((const char*)(a0+k+8), _MM_HINT_T0);
                    _mm_prefetch((const char*)(a1+k+8), _MM_HINT_T0);
                    _mm_prefetch((const char*)(a2+k+8), _MM_HINT_T0);
                    _mm_prefetch((const char*)(a3+k+8), _MM_HINT_T0);
                    _mm_prefetch((const char*)(a4+k+8), _MM_HINT_T0);
                    _mm_prefetch((const char*)(a5+k+8), _MM_HINT_T0);
                }
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* Parallel memset: distribute page faults across threads */
    if(use_mt){
        #pragma omp parallel for schedule(static)
        for(int i=0;i<n;i++) memset(C+(size_t)i*n, 0, (size_t)n*sizeof(double));
    } else {
        ...
        for(int jc=0;jc<n;jc+=nc_blk){
            ...
            for(int pc=0;pc<n;pc+=kc_blk){
                ...
                #pragma omp parallel
                {
                    ...
                    /* Dynamic schedule for large sizes (better load balance) */
...
int main(int argc, char**argv){
    ...
    /* If a size argument is given, run just that size (for comparison scripts) */
    if(argc > 1){
        int n = atoi(argv[1]);
        if(n < 64) n = 64;
        ...
        printf("%4dx%-5d | %8.1f ms | %8.1f ms | %6.1f   | %6.1f   | OK\n",
               n,n,best*1000,best*1000,gf,gf);
        af(A);af(B);af(C);
        return 0;
    }

    printf("====================================================================\n");
    printf("  COM6 v59 - Streaming B-Pack + Dynamic Schedule (%d threads)\n",nth);
    printf("  n<=4096: MC=%d/%d KC=%d/%d NC=%d | n>4096: NC=%d (L3-fit)\n",
           MC_SMALL,MC_LARGE,KC_SMALL,KC_LARGE,NC_DEFAULT,NC_HUGE);
    printf("  Non-temporal B-pack (bypass L1/L2) + dynamic(2) schedule\n");
    ...
    printf("\nStreaming B-pack + dynamic scheduling for load balance\n");
    return 0;
}
