/*
 * COM6 v78 - Prefetch-Optimized Packing
 * =======================================
 * v77 base. Changes:
 *   1. B-pack source prefetch: prefetcht0 next 2 source rows during packing
 *      → hides DRAM latency when B rows are in different pages
 *   2. A-pack source prefetch: prefetch next MR rows' columns ahead
 *   3. B-pack 4x unroll: process 4 rows per iteration (was 2x in v77)
 *      → fewer loop iterations, better instruction-level parallelism
 *
 * i7-10510U: L1=64KB, L2=256KB, L3=8MB shared, 4C/8T, 15W TDP
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v78 com6_v78.c -lm
 */
...
/* Deep KC for 8192+: fewer C passes = less DRAM traffic */
#define KC_HUGE  512
#define MC_HUGE  48
#define NC_HUGE  2048
...
static void pack_B_chunk(const double*__restrict__ B,double*__restrict__ pb,
    ...
    for(int j=j_start;j<j_end;j+=NR){
        ...
        if(nr==NR){
            /* Full NR=8 panel: 4x k-unrolled with source prefetch */
            int k=0;
            for(;k+3<kc;k+=4){
                _mm_prefetch((const char*)(Bkj+4*n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Bkj+4*n+4), _MM_HINT_T0);
                ...
                _mm_prefetch((const char*)(Bkj+5*n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Bkj+5*n+4), _MM_HINT_T0);
                _mm256_store_pd(dest+16,_mm256_loadu_pd(Bkj+2*n));
                _mm256_store_pd(dest+20,_mm256_loadu_pd(Bkj+2*n+4));
                _mm256_store_pd(dest+24,_mm256_loadu_pd(Bkj+3*n));
                _mm256_store_pd(dest+28,_mm256_loadu_pd(Bkj+3*n+4));
                dest+=4*NR; Bkj+=4*n;
            }
            for(;k<kc;k++){
                _mm256_store_pd(dest,_mm256_loadu_pd(Bkj));
...
static void pack_A(const double*__restrict__ A,double*__restrict__ pa,
    ...
    for(int i=0;i<mc;i+=MR){
        ...
        if(mr==MR){
            ...
            int k=0;
            for(;k+3<kc;k+=4){
                /* Prefetch 8 columns ahead on all 6 rows */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    if(n >= 4096){
        /* Single parallel region for large sizes (no fork/join between tiles)
         * + dynamic scheduling for thermal balance on 15W TDP */
        ...
        /* Standard path for 512-2048: per-tile parallel */
        double* pb = aa((size_t)KC_MAX*NC_MAX);
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v78 - Prefetch-Optimized Packing (%d threads)\n",nth);
    ...
    printf("  Double-buffered B-panels for n>=4096, dynamic(2) scheduling\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv78: 4x B-pack unroll + source prefetch in A/B packing.\n");
    printf("Run individual: ./com6_v78 <size> [mt|1t]\n");
    return 0;
}
