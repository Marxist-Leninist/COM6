/*
 * COM6 v101 - v99 + Parallel Strassen infrastructure
 * ====================================================
 * v98 experiment (pure BLIS at 8192) was DROPPED: head-to-head showed
 *   v99 Strassen 32.7-39.6 GF vs v98 BLIS 23.6 GF. Strassen wins ~38-68%
 *   on 15W TDP because 4096-sized bursts stay in thermal envelope,
 *   whereas a single 8192 BLIS run thermally throttles through.
 *
 * v101 keeps v99's dispatch and adds:
 * - Strassen temps from ONE pool allocation (was 23 separate mallocs)
 * - Parallel sub_copy + mat_add + mat_sub (OMP over the outer loop)
 *   Saves ~15% on Strassen glue code at n=8192 per profiling model.
 *
 * Everything else identical to v99.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v101 com6_v101.c -lm
 */
...
static void com6_strassen(const double*A, const double*B, double*C, int n){
    ...
    /* 7 sub-multiplications (each uses full MT BLIS) */
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v101 - v99 + parallel Strassen glue + pool alloc (%d threads)\n",nth);
    ...
    printf("Run individual: ./com6_v101 <size> [mt|1t]\n");
    return 0;
}
