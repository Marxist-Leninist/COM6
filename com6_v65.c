/*
 * COM6 v65 - Adaptive Scheduling Hybrid
 * ======================================
 * Merges v62's wins (C-prefetch, 4x A-pack, dynamic sched) with v26's
 * proven static scheduling for large sizes.
 *
 * Key: schedule(dynamic,1) gives +41% at 2048 but -11% at 4096.
 * Solution: dynamic for n<=2048, static for n>2048.
 *
 * From v62: C-prefetch kernel, 4x A-pack unroll, MC_TINY for 512 MT
 * From v26: static scheduling for 4096+, proven blocking params
 *
 * Blocking:
 *   n<=512:   MC=48,  KC=256 (more IC blocks for 8T load balance)
...
/* C-prefetch 8x-unrolled 6x8 ASM micro-kernel (from v62) */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    if(!use_mt){
        ...
        int use_dynamic = (n <= 2048); /* dynamic helps at <=2048, hurts at 4096+ */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v65 - Adaptive Scheduling Hybrid (%d threads)\n",nth);
    ...
    printf("\nAdaptive scheduling: dynamic(<=2048) + static(>2048)\n");
    printf("Run individual sizes: ./com6_v65 <size> [mt|1t]\n");
    return 0;
}
