/*
 * COM6 v61 - Best-of-all + C-Prefetch + Isolated Benchmarks
 * ==========================================================
 * Combines best techniques from all versions:
 *   - v26 architecture: regular stores (NOT streaming — faster for reused data)
 *   - v60's 4x unrolled A-packing
 *   - C-prefetch: pre-fetch output C rows into cache before micro-kernel writes
 *   - Adaptive 3-tier blocking
 *   - Dynamic scheduling for large sizes
 *   - Single-size CLI mode: ./com6_v61 <size> to avoid thermal cascading
 *
 * Blocking tiers (all fit L2=256KB):
 *   n<=1024:  MC=120, KC=256 → 120*256*8 = 240KB
 *   n<=4096:  MC=96,  KC=320 → 96*320*8  = 240KB
 *   n>4096:   MC=80,  KC=384 → 80*384*8  = 245KB (deep KC for 8192)
 *
 * Compile:
...
/*
 * 8x k-unrolled 6x8 ASM micro-kernel with C-prefetch
 * Prefetches all 6 output rows of C at the start so they're in L1
 * when we do the final load+add+store.
 */
           ...
           double* C, int ldc)
{
    ...
    __asm__ volatile(
        /* Prefetch all 6 rows of C output (64 bytes each = 8 doubles) */
        "prefetcht0 (%[C])\n\t"
        "prefetcht0 32(%[C])\n\t"
        "movq %[C],%%r15\n\t"
        "addq %[ldc],%%r15\n\t"
...
/* Three-tier blocking
 * Key insight: for n>4096, SMALLER KC keeps B-panel in L3.
 * KC=384 → B-panel=2048*384*8=6.3MB (79% of L3=8MB) ← thrashes
 * KC=256 → B-panel=2048*256*8=4.2MB (52% of L3=8MB) ← comfortable
 * MC=120 gives more ic-blocks for 8 threads (ceil(8192/120)=69)
 */
/* Four-tier: tiny MC for 256 so all 8 threads get work */
#define MC_TINY 36
static void get_blocking(int n, int*pMC, int*pKC){
    if(n <= 256){*pMC=MC_TINY;*pKC=KC_SMALL;}
    else if(n <= 1024){*pMC=MC_SMALL;*pKC=KC_SMALL;}
    else if(n <= 2048){*pMC=MC_MED;*pKC=KC_MED;}
    else{*pMC=MC_SMALL;*pKC=KC_SMALL;} /* L3-friendly */
}
        ...
        for(int jc=0;jc<n;jc+=NC){
            ...
            for(int pc=0;pc<n;pc+=kc_blk){
                ...
                #pragma omp parallel
                {
                    ...
                    /* Dynamic scheduling for large sizes: handles thermal imbalance */
                    if(n > 2048){
...
int main(int argc, char**argv){
    ...
    /* Single-size mode: ./com6_v61 <size> [mt|1t] */
    if(argc > 1){
        int n = atoi(argv[1]);
        if(n < 64) n = 64;
        int mt_only = (argc > 2 && strcmp(argv[2],"mt")==0);
        int st_only = (argc > 2 && strcmp(argv[2],"1t")==0);
        size_t nn=(size_t)n*n;
        double*A=aa(nn),*B=aa(nn),*C=aa(nn);
        srand(42);randf(A,n);randf(B,n);

        int runs=(n<=1024)?7:(n<=2048)?5:3;
    ...
    printf("====================================================================\n");
    printf("  COM6 v61 - Best-of-All + C-Prefetch (%d threads)\n",nth);
    printf("  Blocking: n<=1024: MC=%d KC=%d | n<=2048: MC=%d KC=%d | n>2048: MC=%d KC=%d\n",
           MC_SMALL,KC_SMALL,MC_MED,KC_MED,MC_SMALL,KC_SMALL);
    printf("  8x unrolled ASM kernel + C-prefetch, 4x A-pack, dynamic sched >2048\n");
    printf("  TIP: Run ./com6_v61 <size> [mt|1t] for isolated single-size test\n");
    printf("====================================================================\n\n");
    ...
    for(int si=0;si<ns;si++){
        ...
        if(!A||!B||!C2){
            printf("%4dx%-5d | SKIPPED (not enough memory)\n",n,n);
    ...
    printf("\nBest of v26+v60: regular stores, C-prefetch, 4x A-pack, 3-tier blocking\n");
    printf("Run individual sizes with: ./com6_v61 <size> [mt|1t]\n");
    return 0;
}
