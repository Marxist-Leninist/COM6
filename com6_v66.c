/*
 * COM6 v66 - Single Parallel Region + Adaptive Scheduling
 * ========================================================
 * Key change from v65: for large sizes (n>=4096), the parallel region
 * wraps the ENTIRE jc/pc loop nest instead of being created per-tile.
 * At 8192: v65 creates 104 parallel regions (4 JC x 26 PC). v66 creates 1.
 * This eliminates fork/join overhead and keeps threads + caches warm.
 *
 * For small sizes (n<=2048): keeps v65's per-tile parallel region with
 * dynamic scheduling (better load balance).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v66 com6_v66.c -lm
 */
           ...
           double* C, int ldc, int beta)
{
    ...
    __asm__ volatile(
        ...
        "jmp 6f\n\t"
        /* beta=0: store directly (no load, no memset needed) */
        "5:\n\t"
        "vmovupd %%ymm0,(%[C])\n\t"
        "vmovupd %%ymm1,32(%[C])\n\t"
        "addq %[ldc],%[C]\n\t"
...
static void get_blocking_1t(int n, int*pMC, int*pKC){
    if(n <= 1024){*pMC=MC_SMALL;*pKC=KC_SMALL;}
    else{*pMC=MC_LARGE;*pKC=KC_LARGE;}
}

/* Blocking for MT path: 3-tier */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    if(!use_mt){
        get_blocking_1t(n, &mc_blk, &kc_blk);
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v66 - Single Parallel Region + Adaptive Scheduling (%d threads)\n",nth);
    ...
    printf("\nSingle parallel region for 4096+, per-tile dynamic for <=2048\n");
    printf("Run individual sizes: ./com6_v66 <size> [mt|1t]\n");
    return 0;
}
