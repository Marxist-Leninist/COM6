/*
 * COM6 v112 - FAILED EXPERIMENT: two attempts at improving 512 MT
 * =====================================================================
 * v108 at 512 MT measures 93-96 GF (95.2 avg). 1T = 50.9 GF so 8T
 * yields only 1.88x single-core throughput. That looked like barrier
 * overhead was the limiter, so:
 *
 *   Attempt 1 — KC=n one-shot (halve pc-barriers):
 *     Set KC=512 so the pc loop runs once instead of twice, cutting
 *     barrier count in half.
 *     RESULT: 73.7 GF avg (-23% regression vs v108).
 *     Root cause: KC=512 makes PA = MC_MT_SMALL * KC = 48*512 = 192KB
 *     per thread. Comet Lake L2 is 256KB per physical core shared
 *     between 2 HT threads (128KB/HT). PA overflows L2 and every
 *     pack_A read misses to L3. Barrier savings << L2-miss cost.
 *
 *   Attempt 2 — MC=64 for perfect 8-thread balance:
 *     With MC=48, n=512 yields 11 ic blocks / 8 threads = uneven
 *     (7 threads run 1 block, 1 thread runs 4). MC=64 gives exactly
 *     8 ic blocks for n=512, one per thread.
 *     RESULT: 58.2 GF avg (-37% regression vs v108).
 *     Root cause: micro-kernel is 6x8 (MR=6). MC=64 % MR=4 remainder
 *     rows per ic block. The 4-row remainder path in the kernel is
 *     ~30% slower than the full 6-row path, and it fires every block.
 *     Net: balance gain << kernel-edge cost.
 *
 * Structural limit: for n=512 with MR=6, no MC satisfies BOTH
 * (a) MC %% 6 == 0 (clean kernel) and (b) n/MC close to 8 (balance),
 * because 512 = 8*64 and 64 is not a multiple of 6.
 *
 * v108 retained as champion. KC=256, MC=48 is the sweet spot: slight
 * imbalance (1.375 blocks/thread) beats either L2-overflow or kernel
 * remainder-edge overhead.
 *
 * Kept as documented failed experiment.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v112 com6_v112.c -lm
 */
...
#define MC_MT_512 64          /* v112: MC=64 for n<=512 MT (8T load balance) */
...
static void get_blocking_mt(int n, int*pMC, int*pKC){
     ...
     * MC_MT_BAL=64 experiment failed at 4096 MT (64*256=128KB = HT-shared L2).
     * v112: at n<=512 MT, MC=64 gives 8 ic blocks for perfect 8-thread
     * balance. At 512 the per-thread work is so small that imbalance cost
     * dominates; at 4096 the per-thread work amortizes imbalance. */
    if(n <= 512){
        int mc = MC_MT_512;
        const char* env = getenv("COM6_MC_512");
        if(env){ int v = atoi(env); if(v >= 24 && v <= 120 && (v % 6) == 0) mc = v; }
        *pMC = mc;
        *pKC = KC_SMALL;
    }
