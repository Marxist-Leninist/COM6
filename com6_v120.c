/*
 * COM6 v120 - v108 OpenMP proven core + chiplet-aware dispatch
 * =============================================================
 * v119's pthreads pool regressed 28% vs v108 on monolithic (IC-par spin
 * barriers slower than OpenMP's optimized barriers). v120 takes a
 * different approach:
 *
 *   - Monolithic CPUs: EXACT v108 code path (OpenMP IC-par + JC-par).
 *     Zero regression risk — same dispatch, same blocking, same micro-kernel.
 *
 *   - Chiplet CPUs (EPYC, Ryzen with multiple L3 groups): JC-parallel
 *     for ALL sizes (private B panels = zero cross-CCD traffic). Detects
 *     L3 groups via sysfs. Sets OMP threads = physical cores (no HT).
 *     L2-auto-tuned MC for chiplet's larger L2 (512KB on EPYC vs 256KB).
 *
 * New vs v108:
 *   - detect_platform(): L3 groups, physical cores, L2 size detection
 *   - get_mc_for_l2(): auto-tune MC to chiplet's L2 capacity
 *   - Chiplet dispatch: all-JC with physical-core-only threading
 *   - 4x k-unrolled A-packing (minor throughput win from v119)
 *   - 2x k-unrolled B-packing (minor throughput win from v77/v119)
 *   - Strassen opt-in for 8192+ (COM6_USE_STRASSEN=1)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v120 com6_v120.c -lm
 */
 ...
 * ================================================================
 * Monolithic (laptop):
 *   512/1024:  IC-parallel (shared B, cooperative pack)
 *   2048:      JC-parallel (each thread owns columns)
...
int main(int argc, char**argv){
    ...
    printf("COM6 v120 - v108 OpenMP core + chiplet-aware dispatch\n");
    ...
    printf("Dispatch: %s\n\n", g_is_chiplet ?
           ...
           "512/1024 IC-par | 2048 JC-par | 4096+ IC-par+pace (v108 proven)");
    ...
    printf("\nv120: v108 OpenMP core + chiplet detection. Zero regression on monolithic.\n");
    printf("%s, %d physical cores, L2=%dKB.\n",
           g_is_chiplet ? "Multi-CCD" : "Monolithic", g_phys_cores, g_l2_kb);
    printf("COM6_THREADS=N | COM6_FORCE_JC=1 | COM6_FORCE_IC=1 | COM6_USE_STRASSEN=1\n");
    printf("Run individual: ./com6_v120 <size> [mt|1t]\n");
