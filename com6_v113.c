/*
 * COM6 v113 - FAILED EXPERIMENT: JC-parallel at 512 MT
 * =====================================================================
 * Hypothesis: v108 at 512 MT (95 GF, only 1.88x 1T scaling) is
 * barrier-bound on its IC-par path. JC-par has ZERO barriers because
 * each thread owns a disjoint column slab with its own PA/PB buffers.
 * Route 512 MT through com6_multiply_jc (with pre-allocated buffers
 * outside the parallel region to avoid cold-malloc cost).
 *
 * RESULT: 62-81 GF range, consistently lost to v108 when both ran
 * cold. Three-run fair alternation (v113 first each time to give it
 * the cold-CPU advantage):
 *   run 1 cold: v113 62.5 vs v108 88.4 GF (v108 +41%)
 *   run 2 cold: v113 72.7 vs v108 91.7 GF (v108 +26%)
 *   run 3 warm: v113 81.0 vs v108 70.5 GF (v113 wins, but v108 hot)
 *
 * Root cause: cache economics, not barriers.
 *   IC-par: all 8 threads read the SAME 128KB B panel, shared via
 *           L3/L2 cache. Cooperative pack writes once; reads are
 *           cache hits.
 *   JC-par: each of 8 threads reads its OWN 16KB B stripe from main
 *           memory (different columns, no cache sharing). 8x the
 *           effective B-memory bandwidth consumed.
 *
 * For n=512 where the matmul is only ~3ms, memory bandwidth for B
 * dominates. IC-par's shared-B read via L3 cache outweighs its
 * barrier overhead. The barrier cost (~100us * 4 barriers = 400us)
 * is smaller than the bandwidth cost of 8x independent B reads.
 *
 * Conclusion: at 512 MT, v108 IC-par is architecturally superior.
 * The 95 GF ceiling is bandwidth-bound, not barrier-bound.
 *
 * v108 retained as champion. Kept as documented failed experiment.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v113 com6_v113.c -lm
 */
 ...
 * v113: pre-allocate pa/pb buffers OUTSIDE the parallel region to
 * avoid 8x cold-thread malloc on the critical path at small N. */
                              ...
                              int mc_blk, int kc_blk)
{
    ...
    double** pa_arr = (double**)malloc(nthreads * sizeof(double*));
    double** pb_arr = (double**)malloc(nthreads * sizeof(double*));
    for(int t=0; t<nthreads; t++){
        pa_arr[t] = aa((size_t)MC_MAX*KC_MAX);
        pb_arr[t] = aa((size_t)KC_MAX*NC_MAX);
    }
    ...
    #pragma omp parallel
    {
        ...
        if(j_start < n){
            double*pa = pa_arr[tid];
            double*pb = pb_arr[tid];
    ...
    for(int t=0; t<nthreads; t++){ af(pa_arr[t]); af(pb_arr[t]); }
    free(pa_arr); free(pb_arr);
}
                           ...
                           double*__restrict__ C,int n)
{
     ...
     * v113: JC-parallel at n<=512 (zero barrier; each thread owns a
     * 64-col slab). COM6_JC_512=0 disables. */
    if(n >= 4096){
        ...
        int use_jc = 1;
        const char* jc_env = getenv("COM6_JC_512");
        if(jc_env && atoi(jc_env) == 0) use_jc = 0;
        if(use_jc){
            int mc_blk, kc_blk;
            get_blocking(n, &mc_blk, &kc_blk);
            com6_multiply_jc(A, B, C, n, mc_blk, kc_blk);
...
int main(int argc, char**argv){
    ...
    printf("\nv108: pacing extended to 4096 MT (default 150ms inter-NC pace, v107 hoisted parallel)\n");
    printf("COM6_PACE_MS=N tune pace | COM6_PACE_MIN_N=N raise/lower threshold | COM6_IC_2048=1 IC at 2048\n");
    printf("COM6_USE_STRASSEN=1 force v105 Strassen path at 8192+.\n");
    printf("Run individual: ./com6_v108 <size> [mt|1t]\n");
    return 0;
}
