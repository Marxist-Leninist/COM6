/*
 * COM6 v35 - Size-Aware Thread Scaling + AVX-512
 * =================================================
 * Key changes from v35:
 * - Size-aware thread capping for many-core systems (>8 threads)
 *   Formula: 2*sqrt(n/64) caps threads for small sizes, all cores for n>=2048
 * - MC=48 for n<=1024: more ic-blocks for better work-stealing with fewer threads
 ...
 *
 * Compile (AVX-512):
 *   gcc -O3 -march=native -mavx512f -mfma -funroll-loops -o com6_v35 com6_v35.c -lm -lpthread
 *
 * Compile (AVX2 only):
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -o com6_v35 com6_v35.c -lm -lpthread
 */
...
static void get_blocking(int n, int*pMC, int*pKC, int*pNC){
    ...
    else if(n <= 1024) {*pMC=MC_TINY;  *pKC=KC_SMALL; *pNC=NC_DEFAULT;}
                               ...
                               double*__restrict__ C, int n)
{
    ...
    /* Size-aware thread capping for many-core systems (>8 threads).
       Sweep-optimal: 256->4, 512->6, 1024->8, 2048+->all cores.
       Formula: 2*sqrt(n/64) matches sweep data. */
    int active_threads = max_threads;
    if(max_threads > 8 && n < 2048){
        int cap = (int)(2.0 * sqrt((double)n / 64.0) + 0.5);
        if(cap < 2) cap = 2;
                              ...
                              double*__restrict__ C, int n)
{
    int mc_blk, kc_blk, nc_blk;
    get_blocking(n, &mc_blk, &kc_blk, &nc_blk);
    if(n <= 1024){mc_blk=MC_TINY; kc_blk=KC_SMALL; nc_blk=NC_DEFAULT;}
