/*
 * COM6 v81 - L1-Friendly Huge Tier + Persistent Thread Pool
 * =========================================================
 * v80 base with L1-friendly huge tier:
 *   v80 used KC_HUGE=768 → B-panel per NR = 768*8*8 = 49KB > 32KB L1d!
 *   B was streaming from L2 for every micro-kernel call at 8192.
 *   v81 uses KC_HUGE=512 → B-panel = 512*8*8 = 32KB = fits L1 exactly.
 *   More C passes (16 vs 11) but micro-kernel at L1 speed (2x bandwidth).
 *   MC_HUGE=48 → 48/6=8 micro-panels per strip (was 5), better amortization.
 *
 * Compile:
...
/* L1-friendly huge tier: B-panel per NR = 512*8*8=32KB = exactly L1d
 * v80 KC_HUGE=768 overflowed L1 (49KB), forcing B from L2 (~half bandwidth).
 * More C passes (16 vs 11) but each micro-kernel runs at full L1 speed. */
#define KC_HUGE  512
#define MC_HUGE  48     /* 48*512*8=192KB (fits L2 with 64KB headroom) */
#define NC_HUGE  1024   /* 1024*512*8=4.2MB (fits 8MB L3 comfortably) */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v81 - Universal Persistent Thread Pool (%d threads)\n",nth);
