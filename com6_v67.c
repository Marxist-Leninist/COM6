/*
 * COM6 v67 - NT Stores for Beta=0 + Single Parallel Region
 * =========================================================
 * Builds on v66's beta=0 micro-kernel by using non-temporal stores
 * (vmovntpd) on the first KC-tile. NT stores bypass the cache,
 * freeing L3 for the B panel. On subsequent tiles (beta=1), normal
 * stores are used since C data is already in cache from previous tile.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v67 com6_v67.c -lm
 */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v67 - NT Stores for Beta=0 + Single Parallel Region (%d threads)\n",nth);
    ...
    printf("\nNT stores for beta=0, single parallel region for 4096+\n");
    printf("Run individual sizes: ./com6_v67 <size> [mt|1t]\n");
    return 0;
}
