/*
 * COM6 v109 - FAILED EXPERIMENT: finer-grained pacing at 4096
 * =====================================================================
 * Hypothesis: halving NC at n=4096 (2048->1024) triples pace granularity
 * (1 fire -> 3 fires) keeping each compute burst under turbo window.
 *
 * Result: net regression. At matched cooldown (60s on i7-10510U 15W):
 *   v108: 73.5 GF at 4096 MT (NC=2048, 1x 150ms pace)
 *   v109: 57.1 GF at 4096 MT (NC=1024, 3x 75ms pace)
 * A 22% loss despite identical total sleep on paper (225ms vs 150ms +
 * extra B-pack rounds). Likely causes:
 *   1. 75ms is below the scheduler-wakeup + turbo-ramp floor on this CPU.
 *      The pace needs to be long enough for the PCU to ACTUALLY bump
 *      multiplier back to 3.6GHz before new work arrives — 150ms clears
 *      that bar, 75ms does not.
 *   2. Smaller NC = more B-pack rounds = more page-fault/TLB warmup at
 *      each new NC block, eating the savings.
 *   3. Shorter compute bursts under-amortize the OpenMP barrier cost
 *      that bookends every NC block in the hoisted parallel region.
 *
 * Conclusion: v108's 150ms single-fire pacing was already near-optimal.
 * v109 kept as reference; dispatch identical to v108 at NC_4096=2048.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v109 com6_v109.c -lm
 */
 ...
 * v109: nc_blk parameter — caller controls NC granularity per size. */
static void com6_multiply_ic_body(const double*__restrict__ A,
                                   const double*__restrict__ B,
                                   double*__restrict__ C, int n,
                                   int mc_blk, int kc_blk, int nc_blk,
                              ...
                              double*__restrict__ C, int n)
{
    ...
    /* v109: NC defaults to 2048 everywhere except 4096 where 1024 gives
     * finer pacing granularity (3 fires instead of 1). */
    int nc_blk = NC;
    if(n == 4096) nc_blk = 1024;
    const char *nc_env = getenv("COM6_NC_4096");
    if(nc_env && n == 4096){
        int v = atoi(nc_env);
        if(v >= 8 && v <= NC && (v % NR) == 0) nc_blk = v;
    }

    /* v108/v109: pacing extended to n >= 4096.
     * Defaults: 4096 -> 75ms (3 fires, v109), 8192+ -> 400ms. */
    int pace_ms = 0;
    int pace_min_n = 4096;
    const char *min_env = getenv("COM6_PACE_MIN_N");
    if(min_env){ int v = atoi(min_env); if(v > 0 && v <= 16384) pace_min_n = v; }
    if(n >= pace_min_n){
        pace_ms = (n >= 8192) ? 400 : 75;  /* v109: 75ms (3 fires) */
        const char *env = getenv("COM6_PACE_MS");
        if(env){ int v = atoi(env); if(v >= 0 && v <= 10000) pace_ms = v; }
    }
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,nc_blk,pa_bufs,pb,pace_ms);
                                  ...
                                  com6_ctx_t* ctx)
{
     ...
     * v109: ctx pb is sized for NC only (allocated via aa(KC_MAX*NC)), so use
     * NC here — the Strassen pacing-vs-granularity tradeoff is different
     * because sub-muls are already paced externally via Sleep(). */
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,NC,ctx->pa_bufs,ctx->pb,0);
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v109 - Finer-grained pacing at 4096 (%d threads)\n",nth);
    printf("  8192+: IC-par NC=2048 + 400ms pace | 4096: IC-par NC=1024 + 75ms pace x3 (v109) | 2048: JC-par | <2048: IC-par\n");
