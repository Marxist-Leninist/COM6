/*
 * COM6 v124 - SIMD Edge Kernel + Server-Aware Blocking
 * =====================================================
 * v124 base + two key improvements:
 *
 * 1) SIMD-accelerated edge cases:
 *    When nr==NR but mr<MR (the common edge case at the bottom of
 *    each IC strip), use the full 6x8 SIMD micro-kernel into a
 *    temporary buffer instead of scalar fallback. ~8x faster for
 *    edge blocks (scalar loop was MR*NR per k even for mr=2).
 *
 * 2) L2-aware MC for monolithic servers:
 *    Previously, get_mc_for_l2() was only used for chiplet (EPYC).
 *    Now monolithic servers with L2 >= 512KB also use L2-proportional
 *    MC, doubling pB reuse and halving L3 bandwidth demand.
 *    JC-parallel dispatch for large-L2 monolithic at all MT sizes.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v124 com6_v124.c -lm
 */
...
static void detect_platform(void) {
    ...
    /* Auto-tune KC based on L2 size: fill ~85% of L2 with MC×KC A-panel */
    if (g_l2_kb >= 512) {
    ...
    /* Strassen depth: server CPUs can sustain 2-level, thermal-limited can't */
...
static void get_blocking_jc(int n, int*pMC, int*pKC){
    if (g_l2_kb >= 512) {
        *pKC = (n <= 1024) ? KC_SMALL : get_kc_large();
        *pMC = get_mc_for_l2(*pKC);
    } else {
        get_blocking(n, pMC, pKC);
    }
}
...
/* ================================================================
 * STRASSEN: 2-level recursive for 8192+
 * ================================================================
 * Depth 0 (top-level): Strassen at n>=8192
 * Depth 1 (inside Strassen): Strassen at n>=4096
 * Depth 2+: always BLIS (no more recursion)
 *
 * FLOP reduction: (7/8)^2 = 76.6% at 8192 (23.4% savings vs 12.5% at 1 level)
 * Memory: ~480MB peak (384MB level-1 + 96MB level-2, sequential reuse)
 * ================================================================ */
                           ...
                           double*__restrict__ C, int n)
{
    ...
    /* Strassen with platform-adaptive depth:
     * depth < g_max_strassen_depth AND n >= threshold → recurse
     * Monolithic (laptop): max_depth=1, threshold=8192 (same as v122)
     * Chiplet (EPYC):      max_depth=2, threshold=4096 at depth>0 */
    int use_strassen = 0;
    ...
    if (g_is_chiplet) {
        ...
        if(n >= 2048){
            int mc_blk, kc_blk;
            get_blocking_jc(n, &mc_blk, &kc_blk);
            com6_multiply_jc(A, B, C, n, mc_blk, kc_blk);
        } else {
            com6_multiply_ic(A, B, C, n);
        }
...
int main(int argc, char**argv){
    ...
    printf("COM6 v124 - SIMD Edge Kernel + Server-Aware Blocking\n");
    ...
    printf("Strassen: max_depth=%d (%s)\n", g_max_strassen_depth,
           g_max_strassen_depth >= 2 ? "2-level, 23.4% FLOP savings" : "1-level, 12.5% savings");
    printf("Dispatch: %s\n\n", g_is_chiplet ?
           "JC-par all sizes (chiplet)" :
           g_l2_kb >= 512 ?
           "IC-par <2048 | JC-par 2048+ (server, L2-aware MC)" :
           "IC-par 512/1024 | JC-par 2048 | IC-par+pace 4096+ (laptop)");
    ...
    printf("\nv124: SIMD edge kernel + server-aware blocking. Strassen depth=%d, KC=%d.\n",
    ...
    printf("Run individual: ./com6_v124 <size> [mt|1t]\n");
