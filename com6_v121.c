/*
 * COM6 v121 - Static buffer pool + thread warmup
 * ================================================
 * v120's OpenMP core (proven champion) + elimination of per-call
 * allocation overhead:
 *
 *   - Static buffer pool: all per-thread A/B packing buffers allocated
 *     ONCE at startup via ensure_pool(), reused across every GEMM call.
 *     Eliminates ~9MB of _mm_malloc/_mm_free per call (IC path) or
 *     ~46MB across 8 threads (JC path).
 *
 *   - Pre-fault: memset all buffers at allocation time so first GEMM
 *     doesn't pay page-fault overhead.
 *
 *   - Thread warmup: dummy parallel region in detect_platform() to
 *     pre-create the OpenMP thread pool before the first benchmark.
 *
 *   - Eliminated thread-count fork: IC-parallel used to fork a full
 *     parallel region just to query omp_get_num_threads(). Replaced
 *     with omp_get_max_threads() (zero overhead).
 *
 * On a 15W laptop at n=512 (~1ms compute), the per-call allocation
 * overhead was 5-20% of total time. This version reduces it to zero.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v121 com6_v121.c -lm
 */
...
int main(int argc, char**argv){
    ...
    printf("COM6 v121 - Static buffer pool + thread warmup\n");
    ...
    printf("\nv121: Static buffer pool + thread warmup. Zero per-call allocation overhead.\n");
    ...
    printf("Run individual: ./com6_v121 <size> [mt|1t]\n");
