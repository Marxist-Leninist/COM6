/*
 * COM6 v77 - Double-Buffered B + AVX2 A-Pack + Dynamic Scheduling
 * ================================================================
 * v76 base (127.3 GF@4096, 111.9 GF@8192). Changes:
 *   1. Double-buffered B-packing: pack next B panel while computing current
 *      → hides B-pack latency behind compute for large sizes
 *   2. AVX2 A-packing: use SSE2 for 2-wide column loads where possible
 *   3. Dynamic scheduling for 8192+: handles thermal asymmetry across cores
 *   4. Reduced barrier count: single barrier between B-pack and compute
 *   5. L3-aware NT store threshold: only for n>=4096 (not 2048)
 *
 * i7-10510U: L1=64KB, L2=256KB, L3=8MB shared, 4C/8T, 15W TDP
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v77 com6_v77.c -lm
 */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v77 - Double-Buffered B + Dynamic Sched (%d threads)\n",nth);
    ...
    printf("\nv77: Double-buffered B + dynamic scheduling + 2x B-pack unroll.\n");
    printf("Run individual: ./com6_v77 <size> [mt|1t]\n");
    return 0;
}
