/*
 * COM6 v39 - Always-MT + Small-Size MC Tuning
 * =============================================
 * v38 forced single-thread for n<=512, losing badly to OpenBLAS MT.
 * Fix: enable MT for ALL sizes with size-adaptive MC:
 * - n<=512: MC=48 → ceil(256/48)=6 ic-blocks (enough for 8 threads)
 * - n<=1024: MC=120 KC=256
 * - n>1024: MC=96 KC=320
 * - n>2048: NC=3072
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v39 com6_v39.c -lm
 */
...
/* Adaptive blocking */
#define KC_SMALL 256
#define MC_TINY  48     /* For n<=512 MT: more ic-blocks for thread utilization */
#define MC_SMALL 120
#define KC_LARGE 320
#define MC_LARGE 96
#define KC_MAX   320
...
int main(void){
    ...
    printf("====================================================================\n");
    printf("  COM6 v39 - Always-MT + Small-Size MC Tuning (%d threads)\n",nth);
    printf("  n<=512 MT: MC=%d KC=%d (more ic-blocks for threads)\n",MC_TINY,KC_SMALL);
    printf("  512<n<=1024: MC=%d KC=%d | n>1024: MC=%d KC=%d\n",MC_SMALL,KC_SMALL,MC_LARGE,KC_LARGE);
    ...
    printf("\nv39: Always-MT with MC=48 for small sizes + v26 core for large\n");
    return 0;
}
