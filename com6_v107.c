/*
 * COM6 v107 - Hoisted parallel region (1 fork/join per matmul)
 * =====================================================================
 * v106 created a new #pragma omp parallel region inside the kc loop
 * for EACH pc iteration. At 8192 with KC=320 that's 26 pc iters x 4
 * NC blocks = 104 fork/join rounds. Each fork/join ~30us on 8 threads
 * = ~3ms wasted per matmul. At 8192 that's noise (~0.02% of 17s), but
 * at 512 MT (5ms total) 2 forks x 30us = 1.2% and the critical path
 * hits thread startup/shutdown twice.
 *
 * v107: hoist #pragma omp parallel OUTSIDE the jc/pc loops. Threads
 * persist for the entire matmul, sync via barriers between B-pack
 * and ic-loop phases. Sleep pacing wrapped in #pragma omp master +
 * barrier (only master calls Sleep; others wait).
 *
 * Also tried dynamic schedule on ic loop for n<=1024: regressed 12%
 * at 512 MT vs static; static kept throughout.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v107 com6_v107.c -lm
 */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* IC-parallel for 4096+ (pacing is automatic inside com6_multiply_ic when n>=8192) */
    if(n >= 4096){
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v107 -Chunked IC-par w/ thermal pacing (%d threads)\n",nth);
    printf("  8192+: IC-par + inter-NC pace 400ms | 4096: IC-par | 2048: JC-par | <2048: IC-par\n");
    ...
    printf("\nv107: hoisted #pragma omp parallel (1 fork/join per matmul) + v106 pacing at 8192+\n");
    printf("Set COM6_PACE_MS=N to tune pace. Set COM6_USE_STRASSEN=1 to force v105 path.\n");
    printf("Run individual: ./com6_v107 <size> [mt|1t]\n");
    return 0;
}
