/*
 * COM6 v102 - Thermal-paced Strassen (sleep between sub-muls)
 * =====================================================
 * EXPERIMENT: insert short sleeps between the 7 Strassen sub-muls to let
 * the 15W CPU stay in turbo. Hypothesis: 4096 MT at cold ~92 GF; if we
 * give ~500ms-1s thermal recovery between each 1.5s burst, total wall
 * clock could be LESS than v99's continuous 27s at 8192.
 *
 * Model: v99 currently loses ~15s to throttling between steady-state
 *   (~40 GF) and cold (92 GF). If pacing keeps each sub-mul at ~70 GF
 *   effective, 7*(2.0s) + 6*1s_sleep = 20s — faster than 27.7s.
 *
 * Pace duration controlled by COM6_STRASSEN_PACE_MS env var (default 800).
 *
 * v102 = v99 + pacing sleeps. Everything else identical.
 *
...
static void com6_strassen(const double*A, const double*B, double*C, int n){
    ...
    /* 7 sub-multiplications with thermal pacing (v102).
     * Sleep duration from COM6_STRASSEN_PACE_MS env var, default 800ms.
     * 0 disables pacing (equivalent to v99). */
    ...
    com6_multiply(A11, B11, M1, h);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply(A12, B21, M2, h);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply(S4,  B22, M3, h);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply(A22, T4,  M4, h);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply(S1,  T1,  M5, h);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
    com6_multiply(S2,  T2,  M6, h);
    #ifdef _WIN32
    if(pace_ms>0) Sleep(pace_ms);
    #endif
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v102 - Thermal-paced Strassen (%d threads)\n",nth);
    ...
    printf("Run individual: COM6_STRASSEN_PACE_MS=N ./com6_v102 <size> [mt|1t]\n");
    return 0;
}
