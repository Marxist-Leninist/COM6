/*
 * COM6 v49 - Fixed MT load balancing + no-pack beta=0
 * =====================================================
 * v48 bug: manual ic-distribution left threads idle at 512
 *   (11 blocks / 8 threads → 2 threads did nothing!)
 * Fix: use #pragma omp for schedule(dynamic,1) for ic-loop
 *
 * Also: no-pack path now uses beta=0 trick — skip memset,
 * first k iteration stores directly instead of load+add+store.
 *
 * Compile:
...
/* MC_TINY for small MT sizes: more ic-blocks for thread utilization */
#define MC_TINY 48
...
static void get_blocking(int n, int use_mt, int*pMC, int*pKC, int*pNC){
    if(n <= 1024 && use_mt){*pMC=MC_TINY;*pKC=KC_SMALL;*pNC=NC_SMALL;}
 ...
 * Beta=0 optimization: first k iteration stores directly, no memset needed.
 * 8-wide j processing: 2x ymm per j-iteration, contiguous B streaming.
 */
static void com6_multiply_small(const double*__restrict__ A,
                                 const double*__restrict__ B,
                                 double*__restrict__ C, int n)
{
    /* No memset! First k iteration (k=0) stores directly */
    #pragma omp parallel for schedule(static)
    for(int i=0;i<n;i++){
        const double* Ai = A + (size_t)i*n;
        double* Ci = C + (size_t)i*n;

        /* k=0: beta=0 — store directly, no load from C */
        {
            ...
            for(;j<n;j++) Ci[j] = Ai[0] * Bk[j];
        }

        /* k=1..n-1: beta=1 — load C, fmadd, store */
        for(int k=1;k<n;k++){
            ...
            for(;j+3<n;j+=4){
                ...
                c = _mm256_fmadd_pd(ak, _mm256_loadu_pd(&Bk[j]), c);
                _mm256_storeu_pd(&Ci[j], c);
            }
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v49 - Fixed MT Load Balancing (%d threads)\n",nth);
    ...
    printf("  omp for schedule(dynamic,1) + no-pack beta=0\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv49: omp for schedule(dynamic,1) + no-pack beta=0\n");
    return 0;
}
