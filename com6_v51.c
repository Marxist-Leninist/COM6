/*
 * COM6 v51 - NC=2048 for 8192+ & C-Prefetch in Macro Kernel
 * ===========================================================
 * Changes from v50:
 * 1. NC=2048 for ALL sizes including >4096 (was NC_HUGE=1024 for >4096).
 *    B-panel: 2048*320*8=5MB fits 8MB L3. Halves jc iterations for 8192,
 *    cutting A-panel repacking overhead in half (8944 vs 17888 pack_A calls).
 * 2. C-prefetch in macro_kernel: prefetch next ir-block's C rows before
 *    calling micro_6x8, so RFO (read-for-ownership) is initiated early.
 * 3. schedule(dynamic,2) for ic-loop (load-balancing).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v51 com6_v51.c -lm
 */
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v51 - NC=2048 All + C-Prefetch Macro (%d threads)\n",nth);
    printf("  Blocking: <=1024: MC=%d KC=%d NC=%d\n",MC_SMALL,KC_SMALL,NC_STD);
    printf("            >1024:  MC=%d KC=%d NC=%d (all sizes)\n",MC_LARGE,KC_LARGE,NC_STD);
    printf("  schedule(dynamic,2) + C-prefetch in macro_kernel\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv51: NC=2048 all sizes + C-prefetch macro + dynamic,2 (5s cooldown between sizes)\n");
    return 0;
}
