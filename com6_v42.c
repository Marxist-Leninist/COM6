/*
 * COM6 v42 - MC_TINY for Small Sizes + v41 Deep KC
 * ==================================================
 * Adds MC_TINY=48 for n<=512 to fix small-size MT performance.
 * With MC=120 (v41), n=512 had only 5 ic-blocks — not enough for 8 threads.
 * MC=48 gives 11 ic-blocks at 512 → all threads busy → 85 GF (matches BLAS!).
 *
 * 4-tier blocking:
 *   n<=512:  MC=48  KC=256 NC=2048 (11 ic-blocks at 512 for MT)
 *   n<=1024: MC=120 KC=256 NC=2048
 *   n<=4096: MC=96  KC=320 NC=2048
 *   n>4096:  MC=60  KC=512 NC=1024 (deep KC, fewer pc iterations)
 *
 * Other features from v40/v41:
 * - C-prefetch micro-kernel (hides writeback latency)
 * - Single parallel region (no repeated fork/join)
 * - CLI arg for single-size cold testing
 *
 * Compile:
...
/* 8192+ tier: deep KC, MC divisible by MR=6, smaller NC */
#define KC_HUGE  512
#define MC_HUGE  60    /* 10*MR=60, A panel: 60*512*8=240KB fits L2 exactly */
#define NC_HUGE  1024
...
int main(int argc, char**argv){
    ...
    printf("\nv42: Adaptive thread count + v41 deep KC\n");
    return 0;
}
