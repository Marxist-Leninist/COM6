"""COM6 v134 vs OpenBLAS — Fair interleaved comparison
    ...
    for line in r.stdout.split('\n'):
        ...
        if 'GF' in line and ('MT' in line or '1T' in line):
            if '(' in line:
                ...
                for p in parts:
                    if 'GF' in p:
                        try: return float(p.replace('GF)', '').replace('GF', '').strip())
                        except: pass
    return 0

if __name__ == "__main__":
    sizes = [256, 512, 1024, 2048, 4096, 8192]
    exe = os.path.join(os.path.dirname(__file__), "com6_v134.exe")

    print("=" * 80)
    print("  COM6 v134 vs OpenBLAS — Fair Interleaved")
