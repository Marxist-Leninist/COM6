/*
 * COM6 v74 - v73 + Memset Elimination (Two-Kernel Beta)
 * ======================================================
 * v73 base + memset elimination via two micro-kernel variants:
 *   - micro_6x8_beta0: first KC tile — just stores (no load from C, no memset needed)
 *   - micro_6x8_beta1: subsequent KC tiles — load+add+store
 *   - Branch is at macro_kernel call site, NOT inside the kernel (still branch-free)
 *   - Eliminates: (1) memset of entire C matrix, (2) 12 useless loads of zeros per micro-call
 *   - All v73 features: MT-specific blocking (MC=48), 3-tier adaptive blocking,
 *     single parallel region for 4096+
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v74 com6_v74.c -lm
 */
    ...
    for(int jc=0;jc<n;jc+=nc_blk){int nc=(jc+nc_blk<=n)?nc_blk:n-jc;
        for(int pc=0;pc<n;pc+=kc_blk){int kc=(pc+kc_blk<=n)?kc_blk:n-pc;
            ...
            for(int ic=0;ic<n;ic+=mc_blk){int mc=(ic+mc_blk<=n)?mc_blk:n-ic;
                ...
                macro_kernel_1t(pa,pb,C,mc,nc,kc,n,ic,jc,beta);
    }}}
    af(pa);af(pb);
}
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v74 - Memset Elimination + Two-Kernel Beta (%d threads)\n",nth);
    ...
    printf("\nv74: memset elimination + beta0/beta1 kernels + MT-specific blocking.\n");
    printf("Run individual: ./com6_v74 <size> [mt|1t]\n");
    return 0;
}
