/*
 * COM6 v68 - Adaptive NC + NT B-Pack + Deeper Prefetch
 * =====================================================
 * Builds on v67. Changes:
 *   1. Adaptive NC: NC=2048 for n<=2048, NC=1536 for n>2048
 *      At 8192: B-panel 1536*320*8=3.8MB (vs 5MB), leaves more L3 headroom
 *   2. NT stores during B-packing: packed B data is read by other threads,
 *      so use movntpd to avoid polluting packer's L2
 *   3. Deeper prefetch in micro-kernel: 1536 bytes ahead (was 768/1024)
 *   4. Edge-case KC=384 for n=8192 (deeper K reduces pack amortization)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v68 com6_v68.c -lm
 */
...
#define KC_LARGE 320
#define MC_LARGE 96
#define KC_HUGE  384   /* deeper KC for 8192+ */
#define MC_HUGE  72    /* MC*KC*8=72*384*8=216KB, fits L2 */
#define NC_DEFAULT 2048
#define NC_LARGE   1536  /* 1536*320*8=3.8MB, more L3 headroom */
#define KC_MAX 384
#define MC_MAX 120
           ...
           double* C, int ldc, int beta)
{
    ...
    __asm__ volatile(
        ...
        "jmp 6f\n\t"
        /* beta=0: NT store directly (bypass cache, no memset needed) */
        "5:\n\t"
        "vmovntpd %%ymm0,(%[C])\n\t"
        "vmovntpd %%ymm1,32(%[C])\n\t"
        "addq %[ldc],%[C]\n\t"
...
static void get_blocking_1t(int n, int*pMC, int*pKC, int*pNC){
    ...
    else{*pMC=MC_HUGE;*pKC=KC_HUGE;*pNC=NC_LARGE;}
}
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* No memset! beta=0 on first pc-tile handles zeroing in micro-kernel */

    if(!use_mt){
        get_blocking_1t(n, &mc_blk, &kc_blk, &nc_blk);
        double*pa=aa((size_t)MC_MAX*KC_MAX),*pb=aa((size_t)KC_MAX*NC_DEFAULT);
        for(int jc=0;jc<n;jc+=nc_blk){int nc=(jc+nc_blk<=n)?nc_blk:n-jc;
            for(int pc=0;pc<n;pc+=kc_blk){int kc=(pc+kc_blk<=n)?kc_blk:n-pc;
                ...
                if(!beta) _mm_sfence(); /* flush NT stores before beta=1 reads C */
            }
        }
        af(pa);af(pb);
    } else if(n <= 2048){
        ...
        double* pb = aa((size_t)KC_MAX*NC_DEFAULT);
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v68 - Adaptive NC + Deeper Prefetch + KC=384 for 8192 (%d threads)\n",nth);
    printf("  n<=2048: dynamic sched | n>2048: static sched\n");
    printf("  C-prefetch kernel, 4x A-pack, parallel B-pack\n");
    printf("====================================================================\n\n");
    ...
    printf("\nAdaptive NC (1536 for 8192), KC=384/MC=72 for huge, deeper prefetch\n");
    printf("Run individual sizes: ./com6_v68 <size> [mt|1t]\n");
    return 0;
}
