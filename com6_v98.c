/*
 * COM6 v98 - Pure BLIS (no Strassen) at 8192
 * ===========================================
 * Improvements over v97:
 * - DROPPED Strassen at 8192. v97 Strassen gave 42.8 GF; pure BLIS should
 *   give ~100 GF on this 15W laptop. The 8 mat_add passes + 7 recursive
 *   4096-MT calls destroyed cache locality + thermally cooked the CPU.
 *   O(n^2.807) flop savings can't overcome constant-factor cache losses
 *   on cache-tight hardware.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v98 com6_v98.c -lm
 */
...
/* ================================================================
 * MICRO-KERNEL: beta=0 (C = A*B, just store, skip memset)
 * ================================================================ */
static void __attribute__((noinline))
...
#define MC_MT_SMALL 48  /* More work items for small MT (512/48≈11 blocks) */
...
/* ================================================================
 * 1T path: standard BLIS loop nest
 * ================================================================ */
...
/* ================================================================
 * IC-parallel: shared B-panel, parallel B-pack, barrier, parallel IC
 * This is 25% faster than JC-parallel at 2048 because B is packed
 * once and shared, whereas JC has each thread redundantly packing A.
 * ================================================================ */
...
/* ================================================================
 * STRASSEN: single-level Winograd variant for n>=8192
 * ================================================================ */
...
static void com6_strassen(const double*A, const double*B, double*C, int n){
    ...
    /* C12 = U + M5 + M3 */
    mat_add(U,M5,tmp,h); mat_add(tmp,M3,tmp,h);
    sub_write(C,n,0,h,h,tmp);

    /* C21 = U + M7 - M4 */
    mat_add(U,M7,tmp,h); mat_sub(tmp,M4,tmp,h);
    sub_write(C,n,h,0,h,tmp);

    /* C22 = U + M7 + M5 */
    mat_add(U,M7,tmp,h); mat_add(tmp,M5,tmp,h);
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* v98: pure BLIS at all sizes - Strassen disabled. */
    com6_multiply_mt(A,B,C,n);
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v98 - Pure BLIS (no Strassen) (%d threads)\n",nth);
    printf("  n>=512: IC-parallel MT | n<512: 1T\n");
    printf("  1T: MC=%d/%d | MT: MC=%d/%d | KC=%d/%d | NC=%d\n",
           MC_SMALL,MC_LARGE,MC_MT_SMALL,MC_LARGE,KC_SMALL,KC_LARGE,NC);
    ...
    printf("\nv98: Pure BLIS, no Strassen. Cache coherence beats O(n^2.807).\n");
    printf("Dynamic thread limiting: avoids idle threads for small n\n");
    printf("Run individual: ./com6_v98 <size> [mt|1t]\n");
    return 0;
}
