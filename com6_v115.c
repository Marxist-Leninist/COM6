/*
 * COM6 v115 - Size-adaptive HT-aware pinning (pin at n<=1024 only)
 * =====================================================================
 * After v114 was committed as "inconclusive", a properly alternated
 * fair benchmark (each version runs in matched thermal positions, not
 * all-v1 then all-v2) revealed a clear size split:
 *
 *   512 MT:  v114 best 88.8 vs v108 72.0 GF  (+23%, v114 wins)
 *   1024 MT: v114 best 90.3 vs v108 106.7 GF (-15%, v108 wins)
 *   2048 MT: v114 best 64.8 vs v108  71.0 GF (-9%,  v108 wins)
 *
 * Why it splits at ~768 n:
 *   - At 512 MT the matmul is ~3ms. OS-scheduler decisions dominate;
 *     pinning eliminates thread migration cost (cache invalidations).
 *   - At 1024+ the matmul is 12ms+, and the OS scheduler has time to
 *     adapt to other running processes. Hard-pinning forces threads
 *     to wait for their specific core even when another is free.
 *
 * v115 change: pin only when n <= COM6_PIN_MAX (default 1024).
 * Implementation: `com6_multiply_ic_body` takes an extra `int pin`
 * arg; the flag is derived from n at the call site in
 * `com6_multiply_ic`. com6_multiply_jc and _jc_ctx do NOT pin
 * (2048 MT has been shown to regress with pinning).
 *
 * Env:
 *   COM6_NO_PIN=1       disable all pinning (revert to v108)
 *   COM6_PIN_ALL=1      force pinning on for all sizes (= v114 behavior)
 *   COM6_PIN_MAX=<n>    size threshold (default 1024)
 *   COM6_PIN_LINEAR=1   naive tid->cpu mapping (demonstrated regression)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v115 com6_v115.c -lm
 */
...
/* v114: pin each OpenMP thread to a distinct logical CPU on Windows.
 * Called once at the top of every #pragma omp parallel region.
 *
 * Pin mapping spreads threads across PHYSICAL cores first, then HT siblings:
 *   tid 0 -> CPU 0 (core 0, primary)
 *   tid 1 -> CPU 2 (core 1, primary)
 *   tid 2 -> CPU 4 (core 2, primary)
 *   tid 3 -> CPU 6 (core 3, primary)
 *   tid 4 -> CPU 1 (core 0, HT sibling)
 *   tid 5 -> CPU 3 (core 1, HT sibling)
 *   tid 6 -> CPU 5 (core 2, HT sibling)
 *   tid 7 -> CPU 7 (core 3, HT sibling)
 *
 * This matches how NUMA-aware BLAS libraries schedule: fill physical cores
 * with first N=n_physical threads for max FMA throughput, then add HT
 * siblings for the next N threads.
 *
 * Env:
 *   COM6_NO_PIN=1        disable pinning entirely (revert to v108)
 *   COM6_PIN_LINEAR=1    pin tid->tid (legacy v114-a behavior)
 */
static int g_pin_enabled = -1;
static int g_pin_linear  = 0;
static int g_pin_all     = 0;
static int g_pin_max_n   = 1024;
static void pin_thread_to_cpu_init(void){
    if(g_pin_enabled != -1) return;
    const char* no = getenv("COM6_NO_PIN");
    g_pin_enabled = (no && atoi(no) == 1) ? 0 : 1;
    const char* lin = getenv("COM6_PIN_LINEAR");
    g_pin_linear = (lin && atoi(lin) == 1) ? 1 : 0;
    const char* all = getenv("COM6_PIN_ALL");
    g_pin_all = (all && atoi(all) == 1) ? 1 : 0;
    const char* mx = getenv("COM6_PIN_MAX");
    if(mx){ int v = atoi(mx); if(v >= 0 && v <= 16384) g_pin_max_n = v; }
}
static inline int pin_tid_to_cpu(int tid, int nt){
    if(g_pin_linear) return tid;
    /* Assume 2-way HT (4 physical, 8 logical on i7-10510U). */
    int n_phys = nt / 2;
    if(n_phys < 1) n_phys = 1;
    if(tid < n_phys) return tid * 2;         /* primary lanes: 0,2,4,6 */
    else             return (tid - n_phys) * 2 + 1;  /* HT siblings: 1,3,5,7 */
}
/* v115: caller passes `enable` = 1 to opt into pinning (e.g. small-n path),
 * 0 to skip. Respects COM6_NO_PIN (force-off) and COM6_PIN_ALL (force-on). */
static inline void pin_thread_to_cpu_if(int tid, int enable){
#ifdef _WIN32
    if(g_pin_enabled != 1) return;           /* COM6_NO_PIN=1 */
    if(!enable && !g_pin_all) return;         /* size-adaptive: skip large n */
    if(tid < 0 || tid >= 64) return;
    int nt = omp_get_num_threads();
    int cpu = pin_tid_to_cpu(tid, nt);
    DWORD_PTR mask = ((DWORD_PTR)1) << cpu;
    SetThreadAffinityMask(GetCurrentThread(), mask);
#else
    (void)tid; (void)enable;
#endif
}
/* Back-compat wrapper (v114 call sites that force-pin — kept for jc paths
 * behind their own enable flag). v115 call sites should use _if directly. */
static inline void pin_thread_to_cpu(int tid){ pin_thread_to_cpu_if(tid, 1); }
...
{
    /* v115: jc path is only used at n>=2048 where pinning regresses.
     * Pin here only if COM6_PIN_ALL=1 (diagnostic / v114 reproduction). */
    int pin_this = (n <= g_pin_max_n) ? 1 : 0;
    #pragma omp parallel
    {
        int tid = omp_get_thread_num();
        int nt = omp_get_num_threads();
        pin_thread_to_cpu_if(tid, pin_this);
...
static void com6_multiply_jc_ctx(const double*__restrict__ A,
                                  ...
                                  com6_ctx_t* ctx)
{
    /* v115: same pin policy as jc — off at 2048+ (where jc lives). */
    int pin_this = (n <= g_pin_max_n) ? 1 : 0;
    #pragma omp parallel
    {
        int tid = omp_get_thread_num();
                              ...
                              double*__restrict__ C, int n)
{
    ...
    /* v115: pin only when n <= g_pin_max_n (default 1024). At 2048+ MT
     * the matmul is long enough that OS-scheduler flexibility beats
     * hard pinning. */
    int pin_this = (n <= g_pin_max_n) ? 1 : 0;
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,pa_bufs,pb,pace_ms,pin_this);
                                  ...
                                  com6_ctx_t* ctx)
{
     ...
     * Strassen sub-muls at 8192 pass n=4096 — NOT pinned (> g_pin_max_n). */
    int pin_this = (n <= g_pin_max_n) ? 1 : 0;
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,ctx->pa_bufs,ctx->pb,0,pin_this);
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v115 - Size-adaptive HT-aware pinning (%d threads, pin<=n=%d%s)\n",
           nth, g_pin_enabled ? g_pin_max_n : 0,
           g_pin_all ? ", PIN_ALL forced" : "");
    printf("  8192+: IC-par + 400ms pace | 4096: IC-par + 150ms pace | 2048: JC-par | <2048: IC-par+PIN\n");
    ...
    printf("\nv115: v108 + size-adaptive HT-aware pinning (pin iff n<=COM6_PIN_MAX, default 1024)\n");
    printf("COM6_NO_PIN=1 force-off | COM6_PIN_ALL=1 force-on | COM6_PIN_MAX=N set threshold\n");
    printf("COM6_PACE_MS=N | COM6_PACE_MIN_N=N | COM6_IC_2048=1 | COM6_USE_STRASSEN=1 (8192+).\n");
    printf("Run individual: ./com6_v115 <size> [mt|1t]\n");
    return 0;
}
