/*
 * COM6 v63 - 2D Thread Decomposition + L3 NC Tuning
 * ==================================================
 * Based on v62 (proven v26 blocking + C-prefetch).
 * New optimizations:
 *   - 2D thread decomposition: threads own BOTH row-ranges AND column-ranges
 *     Each thread packs + computes a tile of the output matrix.
 *     Reduces false sharing on C and improves cache locality.
 *   - NC tuned per size: NC=2048 for n<=4096, NC=1024 for n>4096
 *     Keeps packed B in L3 (320*1024*8 = 2.6MB vs 8MB L3)
 *   - Private C accumulation for MT to eliminate false sharing entirely
 *
 * Blocking:
 *   n<=1024:  MC=120, KC=256, NC=2048
 *   n>1024:   MC=96,  KC=320, NC=2048
 *   n>4096:   MC=96,  KC=320, NC=1024 (L3-friendly)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v63 com6_v63.c -lm
 */
...
/* 3-tier blocking with NC tuning */
        ...
        for(int jc=0;jc<n;jc+=nc_blk){
            ...
            for(int pc=0;pc<n;pc+=kc_blk){
                ...
                #pragma omp parallel
                {
                    ...
                    /* Dynamic scheduling for better load balance on large n */
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v63 - L3-Tuned NC + C-Prefetch (%d threads)\n",nth);
    printf("  Small(<=1024): MC=%d KC=%d NC=%d\n",MC_SMALL,KC_SMALL,NC_SMALL);
    printf("  Large(<=4096): MC=%d KC=%d NC=%d\n",MC_LARGE,KC_LARGE,NC_LARGE);
    printf("  Huge(>4096):   MC=%d KC=%d NC=%d (B-panel=2.5MB fits L3=8MB)\n",MC_HUGE,KC_HUGE,NC_HUGE);
    printf("  8x ASM kernel + C-prefetch, 4x A-pack, dynamic sched MT\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv26 blocking + C-prefetch + L3-tuned NC for 8192+\n");
    printf("Run individual sizes: ./com6_v63 <size> [mt|1t]\n");
    return 0;
}
