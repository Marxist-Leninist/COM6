/*
 * COM6 v56 - Focused 8192 Tuning: NC=2048 vs Right-Sized Buffers
 * ===============================================================
 * Tests whether NC=4096 actually helps at 8192 when buffers are right-sized.
 * v54 had NC=4096 but allocated NC_MAX=4096 buffers for ALL sizes (TLB hell).
 * This version: right-sized buffers + NC=2048 everywhere (B-panel fits L3).
 *
 * B-panel: 320*2048*8 = 5.1MB (fits 8MB L3) vs 384*4096*8 = 12.6MB (overflows)
 *
 * Also: skip 256/512 (tiny, boring), run 1024-8192 with longer cooldowns.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v56 com6_v56.c -lm
 */
...
/* Two-tier blocking, NC=2048 always (fits L3) */
#define KC_SMALL 256
#define MC_SMALL 120
#define KC_LARGE 320
#define MC_LARGE 96
...
/* 8x k-unrolled 6x8 ASM micro-kernel (identical to v26) */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    if(!use_mt){
        double*pa=aa((size_t)mc_blk*kc_blk),*pb=aa((size_t)kc_blk*NC_ALL);
        for(int jc=0;jc<n;jc+=NC_ALL){int nc=(jc+NC_ALL<=n)?NC_ALL:n-jc;
        ...
        double* pb = aa((size_t)kc_blk*NC_ALL);

        for(int jc=0;jc<n;jc+=NC_ALL){
            int nc=(jc+NC_ALL<=n)?NC_ALL:n-jc;
...
int main(void){
    ...
    printf("====================================================================\n");
    printf("  COM6 v56 - Right-Sized Buffers, NC=2048, C-Prefetch (%d threads)\n",nth);
    printf("  n<=1024: MC=%d KC=%d | n>1024: MC=%d KC=%d | NC=%d always\n",
           MC_SMALL,KC_SMALL,MC_LARGE,KC_LARGE,NC_ALL);
    printf("  B-panel: KC*NC*8 = %.1f MB (fits 8MB L3)\n",KC_LARGE*NC_ALL*8.0/1048576);
    printf("  8x unrolled ASM micro-kernel, static schedule\n");
    printf("====================================================================\n\n");
    ...
    printf("\nRight-sized buffers + NC=2048 (L3-fit) + C-prefetch.\n");
    return 0;
}
