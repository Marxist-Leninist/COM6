/*
 * COM6 v52 - L2-Aware NC Blocking + Reduced Thread Count
 * ========================================================
 * Changes from v51:
 * 1. L2-aware NC for n<=512 MT: NC=128 so B-panel (128*256*8=256KB) fits
 *    exactly in per-core L2. Gives L2-speed B reads instead of L3.
 * 2. Use 4 threads (physical cores only) for n<=512 to avoid HT contention
 *    on tiny workloads. HT threads share FMA units, so 8T doesn't help
 *    compute-bound micro-kernels — it just adds scheduling overhead.
 * 3. NC=2048 for all large sizes (from v51).
 * 4. C-prefetch in macro_kernel (from v51).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v52 com6_v52.c -lm
 */
...
#define NC_L2    128    /* v52: 128*256*8=256KB — fits exactly in per-core L2 */
...
static void macro_kernel(const double*pa,const double*pb,
    ...
    for(int jr=0;jr<nc;jr+=NR){
        ...
        for(int ir=0;ir<mc;ir+=MR){
            ...
            /* v51: Prefetch next ir-block's C rows — initiates RFO early */
            if(ir+MR < mc){
...
static void get_blocking(int n, int use_mt, int*pMC, int*pKC, int*pNC){
    ...
    else if(n <= 512 && use_mt){*pMC=MC_TINY;*pKC=KC_SMALL;*pNC=NC_L2;}  /* v52: B-panel in L2 */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    #endif
    /* v52: Use physical cores only for n<=512 to avoid HT contention */
    ...
    if(!use_mt){
        ...
        /* SINGLE parallel region — v52: num_threads(use_nt) for thread capping */
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v52 - L2-Aware NC + Physical-Core Threading (%d threads)\n",nth);
    ...
    printf("\nv52: L2-aware NC + 4T for small + C-prefetch + 5s cooldown\n");
    return 0;
}
