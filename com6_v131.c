/*
 * COM6 v131 - Persistent Pool + Beta0-NT Stores
 * ===============================================
 * v130 core + non-temporal stores for beta=0 on large matrices:
 *
 * Architecture (same as v130):
 * - Custom persistent pthreads/WinThreads pool for JC-parallel dispatch
 *   (workers spin-wait then condvar/Event sleep, zero fork-join overhead)
 * - OpenMP retained for IC-parallel (shared B + barrier sync) and Strassen helpers
 * - Hybrid dispatch: JC when B+A fits L2, IC when it overflows
 *
 * NEW in v131:
 * 1. Beta0-NT micro-kernel: non-temporal stores (vmovntpd) for first KC tile
 *    on large matrices (n >= 2048). Bypasses cache write-allocate, saving ~35%
 *    write bandwidth when C matrix doesn't fit L3. sfence after each PC tile.
 *
 * 2. g_use_nt flag auto-set based on matrix size vs L3 capacity.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v131 com6_v131.c -lm
 */
...
/* ================================================================
 * MICRO-KERNEL: beta=0 non-temporal (C = A*B, vmovntpd bypasses cache)
 * Saves ~35% write bandwidth when C doesn't fit L3 (avoids write-allocate).
                           ...
                           double*__restrict__ C, int n)
{
    ...
    g_use_nt = ((size_t)n * n * sizeof(double) > (size_t)g_l3_kb * 1024);
...
int main(int argc, char**argv){
    ...
    printf("COM6 v131 - Persistent Thread Pool (zero fork-join overhead)\n");
    ...
    printf("\nv131: persistent pool + beta0-NT stores (spin %d + condvar sleep).\n",
           POOL_SPIN_ITERS);
    printf("IC-parallel via OpenMP. Beta0-NT for C > L3. Strassen depth=%d.\n",
           g_max_strassen_depth);
    ...
    printf("Run individual: ./com6_v131 <size> [mt|1t]\n");
