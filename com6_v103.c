/*
 * COM6 v103 - Persistent packing buffers across Strassen sub-muls
 * ====================================================
 * v99/v101 issue: each of the 7 Strassen sub-multiplies re-allocates
 *   pa_bufs[nthreads] + pb on entry to com6_multiply_ic. At 8192 with
 *   8 threads that's 7 * (8 pa + 1 pb) = 63 alloc/free rounds. Every
 *   allocation is cache-cold and on Windows triggers page faults on
 *   first write, eating first-pass time in each sub-mul.
 *
 * v103 change: allocate the packing buffers ONCE at Strassen entry,
 *   pass them down into *_ctx variants reused across all 7 sub-muls.
 *   - Zero malloc churn between sub-muls
 *   - pa/pb stay warm in L2/L3 across adjacent sub-muls
 *   - No page-fault re-fault per sub-mul
 *
 * v101 pool-alloc (23 Strassen temps in one block) retained.
 * Everything else identical to v99/v101 (same kernels, same Strassen chemistry).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v103 com6_v103.c -lm
 */
...
static void com6_strassen(const double*A, const double*B, double*C, int n){
    ...
    com6_multiply_ctx(A11, B11, M1, h, ctx);   /* M1 = A11*B11 */
    com6_multiply_ctx(A12, B21, M2, h, ctx);   /* M2 = A12*B21 */
    com6_multiply_ctx(S4,  B22, M3, h, ctx);   /* M3 = S4*B22 */
    com6_multiply_ctx(A22, T4,  M4, h, ctx);   /* M4 = A22*T4 */
    com6_multiply_ctx(S1,  T1,  M5, h, ctx);   /* M5 = S1*T1 */
    com6_multiply_ctx(S2,  T2,  M6, h, ctx);   /* M6 = S2*T2 */
    com6_multiply_ctx(S3,  T3,  M7, h, ctx);   /* M7 = S3*T3 */
                              ...
                              double*__restrict__ C, int n)
{
    ...
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,pa_bufs,pb);
                                  ...
                                  com6_ctx_t* ctx)
{
    int mc_blk, kc_blk;
    get_blocking_mt(n, &mc_blk, &kc_blk);
    com6_multiply_ic_body(A,B,C,n,mc_blk,kc_blk,ctx->pa_bufs,ctx->pb);
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v103 - Persistent packing buffers across Strassen sub-muls (%d threads)\n",nth);
    ...
    printf("Run individual: ./com6_v103 <size> [mt|1t]\n");
    return 0;
}
