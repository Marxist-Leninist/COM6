/*
 * COM6 v89 - L1-Safe KC + Physical-Core Threading
 * ================================================
 * BUG FIX: v89's KC=768 for 8192 made A-panel per micro-kernel
 * 6*768*8=36KB > 32KB L1D! Every micro-kernel suffered L1 misses.
 *
 * Key changes:
 * 1. KC capped at 320 for ALL sizes (A-panel=6*320*8=15KB, fits L1D)
 * 2. For n>=4096 MT: use 4 threads (physical cores only, no HT)
 *    → HT threads share FMA units, add no compute throughput
 *    → On 15W TDP laptop, fewer threads = higher sustained clocks
 * 3. NC=2048 for all sizes (B-panel=320*2048*8=5MB fits 8MB L3)
 * 4. MC=96 for large sizes (96 rows / MR=6 = 16 micro-kernels per IC block)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v89 com6_v89.c -lm
 */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v89 - L1-Safe KC + Physical-Core Threading (%d threads)\n",nth);
    ...
    printf("\nv89: Retuned L3 blocking + prefetch A-packing.\n");
    printf("Run individual: ./com6_v89 <size> [mt|1t]\n");
    return 0;
}
