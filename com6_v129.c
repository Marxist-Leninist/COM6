/*
 * COM6 v129 - Adaptive Thread Count + Staggered Prefetch
 * ======================================================
 * v128 core with targeted fixes for regressions:
 *
 * 1. Adaptive thread count per size:
 *    - n<=512: physical cores only (higher sustained turbo under 15W TDP)
 *    - n>=1024: all logical threads (HT helps hide pack latency)
 *    v128 globally pinned to phys cores, hurting 1024 (-16%) and 2048 (-6%).
 *
 * 2. MC=72 for 1024 IC-parallel (was 48):
 *    With 8 threads: ceil(1024/72)=15 tiles, 15/8=~2 per thread.
 *    Was 21 tiles with MC=48 -> more scheduling overhead.
 *
 * 3. Staggered micro-kernel prefetch:
 *    Distribute A/B prefetches across ranks 0,2,4,6 instead of
 *    bulk at ranks 0 and 4. Smoother memory pipeline feeding.
 *
 * 4. Compile WITHOUT -flto (was in v128, may hurt inline ASM register alloc).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v129 com6_v129.c -lm
 */
...
static int get_mt_threads(int n) {
    ...
    if (g_phys_cores <= 0 || g_phys_cores >= g_logical_cores)
        return g_logical_cores;
    if (n <= 512) return g_phys_cores;
    return g_logical_cores;
}

/* ================================================================
 * MICRO-KERNEL: beta=1 (C += A*B) with staggered prefetch
 * ================================================================ */
...
micro_6x8_beta1(int kc, const double* pA, const double* pB, double* C, int ldc)
{
    ...
    __asm__ volatile(
        ...
        /* rank 0: prefetch A next-next cache line */
        "prefetcht0 384(%[pA])\n\t"
...
#define RANK1(AO,BO) \
        ...
        RANK1(96,128)
        RANK1(144,192)
        /* rank 4: prefetch A far ahead */
        "prefetcht0 768(%[pA])\n\t"
        RANK1(192,256)
        RANK1(240,320)
        /* rank 6: prefetch B far ahead */
        "prefetcht0 1024(%[pB])\n\t"
        RANK1(288,384)
        RANK1(336,448)
#undef RANK1
...
/* ================================================================
 * PACKING — 4x k-unrolled B-pack, 4x k-unrolled A-pack with 6-row prefetch
 * ================================================================ */
...
static void get_blocking_mt(int n, int*pMC, int*pKC){
    if (g_is_chiplet) {
        ...
        *pMC = get_mc_for_l2(*pKC);
        ...
        if(n <= 1024){*pMC=MC_MT_MED;*pKC=KC_SMALL;}
...
static void com6_multiply_jc(const double*__restrict__ A,
                              const double*__restrict__ B,
                              double*__restrict__ C, int n,
                              int mc_blk, int kc_blk, int nthreads)
{
#ifdef _OPENMP
    int nc_blk = g_nc_jc;
                           ...
                           double*__restrict__ C, int n)
{
    ...
    if (fjc && atoi(fjc) == 1) {
        int mc_blk, kc_blk;
        get_blocking_mt(n, &mc_blk, &kc_blk);
        com6_multiply_jc(A, B, C, n, mc_blk, kc_blk, nthreads);
        return;
    }
    if (fic && atoi(fic) == 1) {
    ...
    if (g_is_chiplet) {
        ...
        int bpanel_per_thread = (n / nthreads) * kc * (int)sizeof(double);
...
int main(int argc, char**argv){
    ...
    if(argc >= 2){
        ...
        if(n>=16384) runs=1;
        int mt_threads = get_mt_threads(n);
    ...
    printf("COM6 v129 - Adaptive Thread Count + Staggered Prefetch\n");
    printf("Threads: phys=%d, logical=%d (L2: %dKB, L3: %dKB, %s)\n",
           g_phys_cores, g_logical_cores, g_l2_kb, g_l3_kb, g_is_chiplet ? "CHIPLET" : "MONOLITHIC");
    ...
    printf("Thread policy: <=512 -> %d phys | >=1024 -> %d logical\n",
           g_phys_cores, g_logical_cores);
    printf("\n Size      |   1T (ms)  |   MT (ms)  |  GF(1T) |  GF(MT) |  T | Verify\n");
    printf("-----------|------------|------------|---------|---------|----|---------\n");
    ...
    printf("\nv129: adaptive threads (phys=%d<=512, logical=%d>=1024), MC_MT_MED=%d for 1024.\n",
           g_phys_cores, g_logical_cores, MC_MT_MED);
    printf("Staggered prefetch: A@rank0,4 B@rank2,6. Strassen depth=%d, KC=%d.\n",
           g_max_strassen_depth, kc_l);
    printf("%s, L2=%dKB, L3=%dKB.\n",
           g_is_chiplet ? "Multi-CCD" : "Monolithic", g_l2_kb, g_l3_kb);
    printf("COM6_THREADS=N | COM6_STRASSEN_DEPTH=N | COM6_NO_STRASSEN=1\n");
    printf("Run individual: ./com6_v129 <size> [mt|1t]\n");
