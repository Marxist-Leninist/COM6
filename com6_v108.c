/*
 * COM6 v108 - Pacing extended to 4096 MT (thermal-window for 15W TDP)
 * =====================================================================
 * v107 applied per-NC thermal pacing only at n>=8192. At 4096 MT the
 * full burst is ~2.3s on a 15W i7-10510U, which already pushes the CPU
 * out of turbo by ~half-way through. The full-suite measurement shows
 * 49.6 GF at 4096 MT vs ~92 GF cold-CPU peak — a >45% thermal drop.
 *
 * v108: enable pacing at 4096+ (was 8192+ in v107). Default pace is
 * 150ms at 4096 (shorter than 8192's 400ms since 4096 has fewer NC
 * blocks). With NC=2048 and n=4096, there are 2 NC iterations, so
 * pacing fires 1x mid-burst — enough to let turbo recover without
 * dominating total wall time.
 *
 * Env controls:
 *   COM6_PACE_MS=<ms>     override pace value (0 disables)
 *   COM6_PACE_MIN_N=<n>   override threshold (default 4096 in v108)
 *
 * Also tried IC-parallel dispatch at n=2048 (v108-a prototype) — it
 * regressed vs JC-par in warm-CPU runs, so dispatch is unchanged.
 * A COM6_IC_2048=1 env flag keeps the experiment reachable.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static \
 *       -o com6_v108 com6_v108.c -lm
 */
