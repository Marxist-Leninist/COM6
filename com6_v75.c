/*
 * COM6 v75 - v74 + NT Stores for Large Beta=0
 * ==============================================
 * v74 base + non-temporal stores in beta=0 kernel for n>=2048:
 *   - Three kernel variants: beta1 (load+add+store), beta0 (just store),
 *     beta0_nt (non-temporal store, bypasses cache)
 *   - NT stores avoid RFO (read-for-ownership) on cache lines we're writing for first time
 *   - At 8192, C=512MB — NT stores keep L3 free for A/B panels
 *   - Threshold: use NT for n>=2048 (C>32MB, won't fit in L3)
 *   - All v74 features: memset elimination, MT-specific blocking, 3-tier blocking
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v75 com6_v75.c -lm
 */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v75 - NT Stores for Large Beta=0 (%d threads)\n",nth);
    printf("  n<=1024: MC=%d KC=%d NC=%d\n",MC_SMALL,KC_SMALL,NC_DEFAULT);
    printf("  n<=4096: MC=%d KC=%d NC=%d\n",MC_LARGE,KC_LARGE,NC_DEFAULT);
    printf("  n>=8192: MC=%d KC=%d NC=%d (B-panel 4.7MB, L3-fit)\n",MC_HUGE,KC_HUGE,NC_HUGE);
    printf("  Branch-free kernel, 4x A-pack, single parallel region 4096+\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv75: NT stores for large beta=0 + memset elimination + MT-specific blocking.\n");
    printf("Run individual: ./com6_v75 <size> [mt|1t]\n");
    return 0;
}
