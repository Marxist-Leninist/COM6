/*
 * COM6 v117 - Merge v100 pthreads pool + v108 thermal pacing (definitive)
 * ========================================================================
 * v108 is the proven all-sizes champion (5/5 vs OpenBLAS, 1.08-1.66x).
 * v100 proved a ~14% win at 512 MT via persistent pthreads pool (~1us
 * dispatch vs OpenMP's ~50-100us fork-join).
 *
 * v117 merges the best of both:
 * - v100's persistent pthreads pool (spin-wait + condvar, ~1us dispatch)
 * - v108's thermal pacing at n>=4096 (all threads sleep between NC blocks)
 * - v108's dispatch: IC-parallel at 8192 by default (Strassen optional)
 * - v108's second prefetch wave in micro-kernels
 * - Atomic work-stealing for IC-blocks, zero-barrier JC-parallel at 2048
 *
 * Dispatch: 8192+ IC-par+pacing | 4096 IC-par+pacing | 2048 JC-par | <2048 IC-par
 *
 * Env controls:
 *   COM6_PACE_MS=<ms>         override pace value (0 disables)
 *   COM6_PACE_MIN_N=<n>       override pacing threshold (default 4096)
 *   COM6_USE_STRASSEN=1       force Strassen at 8192+ (default: IC-par)
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -lpthread -o com6_v117 com6_v117.c -lm
 */
...
static void pool_init(void) {
    ...
    for (int i = 0; i < ncpus; i++) {
        pthread_create(&g_pool.threads[i], NULL, pool_worker, (void*)(intptr_t)i);
    }
}
                           ...
                           double*__restrict__ C, int n)
{
    ...
    /* v117: IC-parallel with pacing at 8192 by default (from v108 finding:
     * IC-par+pacing sustains clocks better than Strassen on 15W laptop).
     * COM6_USE_STRASSEN=1 opts into Strassen for comparison. */
    ...
    if (n >= 4096) {
         ...
         * private column slice + B-panel. Proven better than IC-parallel
         * at 512/1024/2048 with pthreads pool (no sync overhead). */
        int mc_blk, kc_blk;
        get_blocking_mt(n, &mc_blk, &kc_blk);
...
int main(int argc, char**argv){
    ...
    printf("COM6 v117 - pthreads pool + thermal pacing (v100+v108 merge)\n");
    printf("Threads: %d (persistent pool, ~1us dispatch)\n", nth);
    printf("Dispatch: 4096+ IC-par+pacing | <4096 JC-par (zero barriers) | Strassen via COM6_USE_STRASSEN=1\n\n");
    ...
    printf("\nv117: pthreads pool (~1us dispatch) + thermal pacing (v108). Merge of v100+v108.\n");
    printf("COM6_PACE_MS=N tune pace | COM6_PACE_MIN_N=N threshold | COM6_USE_STRASSEN=1\n");
    printf("Run individual: ./com6_v117 <size> [mt|1t]\n");
