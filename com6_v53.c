/*
 * COM6 v53 - Prefetch-Optimized Packing
 * ======================================
 * Changes from v53:
 * 1. Pack_B source prefetch: For large n, B rows are stride-n*8 bytes apart
 *    (64KB at 8192). HW prefetcher can't handle >4KB strides on Comet Lake.
 *    Add _mm_prefetch of next B row during packing to hide DRAM latency.
 * 2. Pack_B 2x k-unrolling: Process 2 rows per loop iteration, interleaving
 *    loads (from scattered B) with stores (to contiguous pb). Better ILP.
 * 3. Pack_A full 6-row prefetch: v52 prefetches only a0,a3 (2 of 6 rows).
 *    Now prefetch all 6 rows for deeper look-ahead on strided accesses.
 * 4. Macro_kernel: prefetch all 6 C rows (was 3 in v52) for RFO warm-up.
 *
 * These are packing-focused optimizations. For large sizes (4096+) where
 * packing is 15-20% of total time, even a 2x packing speedup = 8-10% total.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v53 com6_v53.c -lm
 */
...
#define NC_STD   2048   /* 2048*320*8=5MB — works for 1T or moderate MT */
#define NC_HUGE  1024   /* 1024*320*8=2.5MB — leaves L3 room for 8 threads at 8192 */
#define NC_L2    128    /* v53: 128*256*8=256KB — fits exactly in per-core L2 */
...
/* MC_TINY for small MT: more ic-blocks for thread utilization */
#define MC_TINY  48
#define MC_MICRO 30   /* n<=256 MT: 256/30=9 blocks → all 8 threads busy, MR-aligned */
...
micro_6x8(int kc, const double* pA, const double* pB, double* C, int ldc, int beta)
{
    ...
    __asm__ volatile(
        ...
        /* Distributed prefetch: pA every 2 k-steps, pB every 2 k-steps, staggered */
        "prefetcht0 768(%[pA])\n\t"   /* pA: 2 iterations ahead */
        ...
        /* beta=0: C = accumulators (no load needed) */
        "6:\n\t"
        "vmovupd %%ymm0,(%[C])\n\t"
        "vmovupd %%ymm1,32(%[C])\n\t"
        "addq %[ldc],%[C]\n\t"
...
/* v53: 2x k-unrolled B-packing with source prefetch */
static void pack_B_chunk(const double*__restrict__ B,double*__restrict__ pb,
    ...
    for(int j=js;j<je;j+=NR){
        ...
        if(nr==NR){
            int k=0;
            /* 2x k-unrolled with prefetch 2 rows ahead */
            for(;k+1<kc;k+=2){
                _mm_prefetch((const char*)(Bkj+2*n), _MM_HINT_T0);
                _mm_prefetch((const char*)(Bkj+2*n+4), _MM_HINT_T0);
                _mm256_store_pd(d,  _mm256_loadu_pd(Bkj));
                _mm256_store_pd(d+4,_mm256_loadu_pd(Bkj+4));
                _mm256_store_pd(d+8,  _mm256_loadu_pd(Bkj+n));
                _mm256_store_pd(d+12,_mm256_loadu_pd(Bkj+n+4));
                d+=2*NR;Bkj+=2*n;
            }
            for(;k<kc;k++){
                _mm256_store_pd(d,_mm256_loadu_pd(Bkj));
...
static void pack_A(const double*__restrict__ A,double*__restrict__ pa,
    ...
    for(int i=0;i<mc;i+=MR){
        ...
        if(mr==MR){
            ...
            int k=0;
            for(;k+3<kc;k+=4){
                /* v53: prefetch all 6 rows 16 elements ahead */
                _mm_prefetch((const char*)(a0+k+16), _MM_HINT_T0);
                _mm_prefetch((const char*)(a1+k+16), _MM_HINT_T0);
                _mm_prefetch((const char*)(a2+k+16), _MM_HINT_T0);
                _mm_prefetch((const char*)(a3+k+16), _MM_HINT_T0);
                _mm_prefetch((const char*)(a4+k+16), _MM_HINT_T0);
                _mm_prefetch((const char*)(a5+k+16), _MM_HINT_T0);
...
static void macro_kernel(const double*pa,const double*pb,
    ...
    for(int jr=0;jr<nc;jr+=NR){
        ...
        for(int ir=0;ir<mc;ir+=MR){
            ...
            /* v53: Prefetch ALL 6 C rows of next ir-block — initiates RFO early */
            if(ir+MR < mc){
...
static void get_blocking(int n, int use_mt, int*pMC, int*pKC, int*pNC){
    if(n <= 256 && use_mt){*pMC=MC_MICRO;*pKC=n;*pNC=n;}  /* Single panel: KC=n, NC=n */
    else if(n <= 512 && use_mt){*pMC=MC_TINY;*pKC=KC_SMALL;*pNC=NC_L2;}  /* v53: B-panel in L2 */
    else if(n <= 1024 && use_mt){*pMC=MC_TINY;*pKC=KC_SMALL;*pNC=NC_STD;}
    else if(n <= 1024){*pMC=MC_SMALL;*pKC=KC_SMALL;*pNC=NC_STD;}
    else if(n <= 4096){*pMC=MC_LARGE;*pKC=KC_LARGE;*pNC=NC_STD;}
    else{*pMC=MC_LARGE;*pKC=KC_LARGE;*pNC=NC_STD;}  /* v51: NC=2048 for 8192+ (was NC_HUGE=1024) */
}

/*
 * No-pack multiply for small matrices (n<=256).
 * 4-wide k-unrolled inner loop for better ILP.
 * Uses all threads — fork cost is amortized by schedule(static).
 */
static void com6_multiply_small(const double*__restrict__ A,
                                 ...
                                 double*__restrict__ C, int n)
{
    ...
    for(int i=0;i<n;i++){
        ...
        /* k=0: store directly (beta=0) */
        {
            __m256d ak = _mm256_broadcast_sd(&Ai[0]);
            const double* Bk = B;
            for(int j=0;j<n;j+=8){
                _mm256_storeu_pd(&Ci[j],   _mm256_mul_pd(ak, _mm256_loadu_pd(&Bk[j])));
                _mm256_storeu_pd(&Ci[j+4], _mm256_mul_pd(ak, _mm256_loadu_pd(&Bk[j+4])));
            }
        }

        /* k=1..n-1: 4x unrolled k for ILP — 4 broadcasts interleaved */
        int k=1;
        for(;k+3<n;k+=4){
            __m256d ak0 = _mm256_broadcast_sd(&Ai[k]);
            __m256d ak1 = _mm256_broadcast_sd(&Ai[k+1]);
            __m256d ak2 = _mm256_broadcast_sd(&Ai[k+2]);
            __m256d ak3 = _mm256_broadcast_sd(&Ai[k+3]);
            const double* B0 = B + (size_t)k*n;
            ...
            for(int j=0;j<n;j+=8){
                ...
                c0 = _mm256_fmadd_pd(ak0, _mm256_loadu_pd(&B0[j]), c0);
                c1 = _mm256_fmadd_pd(ak0, _mm256_loadu_pd(&B0[j+4]), c1);
                c0 = _mm256_fmadd_pd(ak1, _mm256_loadu_pd(&B1[j]), c0);
                c1 = _mm256_fmadd_pd(ak1, _mm256_loadu_pd(&B1[j+4]), c1);
                c0 = _mm256_fmadd_pd(ak2, _mm256_loadu_pd(&B2[j]), c0);
                c1 = _mm256_fmadd_pd(ak2, _mm256_loadu_pd(&B2[j+4]), c1);
                c0 = _mm256_fmadd_pd(ak3, _mm256_loadu_pd(&B3[j]), c0);
                c1 = _mm256_fmadd_pd(ak3, _mm256_loadu_pd(&B3[j+4]), c1);
                _mm256_storeu_pd(&Ci[j], c0);
                _mm256_storeu_pd(&Ci[j+4], c1);
            }
        }
        for(;k<n;k++){
            __m256d ak = _mm256_broadcast_sd(&Ai[k]);
            const double* Bk = B + (size_t)k*n;
            for(int j=0;j<n;j+=8){
                __m256d c0 = _mm256_loadu_pd(&Ci[j]);
                __m256d c1 = _mm256_loadu_pd(&Ci[j+4]);
                c0 = _mm256_fmadd_pd(ak, _mm256_loadu_pd(&Bk[j]), c0);
                c1 = _mm256_fmadd_pd(ak, _mm256_loadu_pd(&Bk[j+4]), c1);
                _mm256_storeu_pd(&Ci[j], c0);
                _mm256_storeu_pd(&Ci[j+4], c1);
            }
        }
 ...
 * v49 fix: use #pragma omp for schedule(dynamic,1) for ic-loop
 * instead of manual distribution that left threads idle.
 */
static void com6_multiply(const double*__restrict__ A,
                           const double*__restrict__ B,
                           double*__restrict__ C,int n)
{
    /* Small-matrix fast path: no packing, direct SIMD, reduced threads */
    if(n <= 256){
        com6_multiply_small(A, B, C, n);
        return;
    }
    ...
    #endif
    /* v53: Use physical cores only for n<=512 to avoid HT contention */
    int use_nt = (n <= 512 && nthreads > 4) ? 4 : nthreads;
    ...
    if(!use_mt){
        ...
        double** pa_bufs = (double**)malloc(use_nt * sizeof(double*));
        for(int t=0;t<use_nt;t++) pa_bufs[t] = aa((size_t)MC_MAX*KC_MAX);
        double* pb = aa((size_t)KC_MAX*NC_MAX);

        /* SINGLE parallel region — v53: num_threads(use_nt) for thread capping */
        #pragma omp parallel num_threads(use_nt)
        {
            ...
            for(int jc=0;jc<n;jc+=nc_blk){
                ...
                for(int pc=0;pc<n;pc+=kc_blk){
                    ...
                    /* v49: proper load-balanced ic-loop using omp for
                     * Fixes v48 bug where manual distribution left threads idle
                     * (e.g., 512/48=11 blocks, ceil(11/8)=2 → threads 6,7 got nothing)
                     */
                    int beta = (pc > 0) ? 1 : 0;
                    int n_ic = (n + mc_blk - 1) / mc_blk;

                    #pragma omp for schedule(dynamic,2) nowait
                    for(int ic_idx=0; ic_idx<n_ic; ic_idx++){
                        int ic = ic_idx * mc_blk;
        ...
        for(int t=0;t<use_nt;t++) af(pa_bufs[t]);
        free(pa_bufs);
        af(pb);
    }
}
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v53 - L2-Aware NC + Physical-Core Threading (%d threads)\n",nth);
    printf("  Blocking: <=256 MT: MC=%d KC=n NC=n\n",MC_MICRO);
    printf("            <=512 MT: MC=%d KC=%d NC=%d (B-panel in L2, 4 threads)\n",MC_TINY,KC_SMALL,NC_L2);
    printf("            <=1024:   MC=%d/%d KC=%d NC=%d\n",MC_TINY,MC_SMALL,KC_SMALL,NC_STD);
    printf("            >1024:    MC=%d KC=%d NC=%d\n",MC_LARGE,KC_LARGE,NC_STD);
    printf("  schedule(dynamic,2) + C-prefetch + L2-fit B-panels + 5s cooldown\n");
    printf("====================================================================\n\n");

    /* Warm up OpenMP thread pool — eliminates first-call fork overhead */
    {
        double *tw = aa(64*64), *tw2 = aa(64*64), *tw3 = aa(64*64);
        randf(tw,64); randf(tw2,64);
        com6_multiply(tw, tw2, tw3, 64);
        af(tw); af(tw2); af(tw3);
    }
    ...
    for(int si=0;si<ns;si++){
        ...
        /* v51: 5-second cooldown between sizes to combat thermal throttling on laptops */
        if(!single_mode && si < ns-1){
            fflush(stdout);
            struct timespec ts = {5, 0};
            nanosleep(&ts, NULL);
        }
    }
    printf("\nv53: L2-aware NC + 4T for small + C-prefetch + 5s cooldown\n");
    return 0;
}
