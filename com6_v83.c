/*
 * COM6 v83 - Universal Persistent Thread Pool
 * ============================================
 * v79 base + v31's persistent threading for ALL MT sizes.
 * v79 used single parallel region only for n>=4096, and per-tile
 * parallel for 512-2048 — that's (n/KC)*(n/NC) fork-joins per multiply.
 * v80 uses ONE parallel region for all MT sizes.
 *
 * This gave v31 a 14% win at 2048 and 13% at 4096 over v79.
 * Combined with v79's beta0/beta1 kernels, memset elimination,
 * deep KC=768 for 8192, and MC_MT=48 for small MT.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v80 com6_v80.c -lm
 */
...
/* Ultra-deep KC for 8192+: minimize C passes (8 vs 11 with KC=768)
 * KC=1024, MC=24: 24*1024*8=192KB (fits L2, 64KB headroom)
 * B per NR: 1024*8*8=64KB (exceeds L1, streams from L2 — acceptable)
 * C passes: 8192/1024=8 vs 8192/768=11 → 27% fewer C round-trips */
#define KC_HUGE  1024
#define MC_HUGE  24     /* 24*1024*8=192KB (fits L2 with headroom) */
#define NC_HUGE  768    /* 768*1024*8=6.3MB (fits 8MB L3) */
...
int main(int argc, char**argv){
    ...
    /* Full benchmark */
    printf("====================================================================\n");
    printf("  COM6 v83 - Universal Persistent Thread Pool (%d threads)\n",nth);
    ...
    printf("  Universal persistent thread pool (1 fork-join for ALL sizes)\n");
    printf("====================================================================\n\n");
    ...
    printf("\nv80: Persistent threading for all sizes + v79's deep KC.\n");
    printf("Run individual: ./com6_v80 <size> [mt|1t]\n");
    return 0;
}
