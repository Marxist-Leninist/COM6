/*
 * COM6 v130 - Persistent Thread Pool (eliminates OpenMP fork-join overhead)
 * ========================================================================
 * v129 core with custom persistent pthreads/WinThreads pool for JC-parallel:
 *
 * THE PROBLEM: OpenMP `#pragma omp parallel` has ~15-20us fork-join overhead
 * per invocation (thread wakeup + implicit barrier). OpenBLAS uses persistent
 * threads that spin-wait between calls, giving <1us dispatch. At 512x512
 * (~2ms compute), this overhead matters.
 *
 * THE FIX:
 * 1. Custom persistent thread pool for JC-parallel dispatch:
 *    - Workers spin-wait (2000 iterations, ~1us) then sleep on condvar/Event
 *    - Master thread participates in computation (thread 0)
 *    - Zero barriers needed - each thread owns its column range
 *    - ~1-3us dispatch when spinning, ~5us when sleeping
 *    - Thermal-friendly: workers auto-sleep after spin window expires
 *
 * 2. OpenMP retained for IC-parallel (needs barriers for B-pack sync)
 *    and Strassen helper functions (quad_add, quad_sub, etc.)
 *
 * 3. Dispatch policy (laptop, L2=256KB):
 *    - 512: JC via pool (B+A=224KB fits L2, zero barriers)
 *    - 1024: IC via OpenMP (B+A=352KB overflows L2, need shared B)
 *    - 2048: JC via pool (barrier elimination wins over L2 overflow)
 *    - 4096+: IC via OpenMP (thermal pacing between JC tiles)
 *
 * 4. Removed buffer memset from pool init (beta=0 kernel never reads C).
 *
 * 5. All 6 A-row prefetch in pack_A (v129 only prefetched a0).
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp \
 *       -o com6_v130 com6_v130.c -lm
 *   # -fopenmp still needed for IC-parallel and Strassen helpers
 */
...
int main(int argc, char**argv){
    ...
    printf("COM6 v130 - Persistent Thread Pool (zero fork-join overhead)\n");
    ...
    printf("Dispatch: JC-pool when B+A fits L2, IC-OpenMP otherwise\n\n");
    ...
    printf("\nv130: persistent pthreads pool for JC-parallel (spin %d + condvar sleep).\n",
           POOL_SPIN_ITERS);
    printf("IC-parallel via OpenMP for 1024 and 4096+. Strassen depth=%d.\n",
           g_max_strassen_depth);
    ...
    printf("Run individual: ./com6_v130 <size> [mt|1t]\n");
