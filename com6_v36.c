/*
 * COM6 v36 - Sweep-Optimized Thread Scaling + KC Tuning
 * ================================================================
 * Key changes from v35:
 * - Table-based thread capping (from parameter sweep):
 *   256->4, 512->6, 1024->12, 2048+->all cores
 *   (v35 used 2*sqrt(n/64) which gave only 8 at 1024)
 * - KC=320 for n=1024: reduces pc iterations from 4 to 3.2 (+32% at 1024)
 * - Combined: 1024 jumps from 123 GF to 163 GF (beats BLAS 161.8 GF)
 * - MC=48 for n<=1024: more ic-blocks for better work-stealing
 * - Thermal monitoring disabled for server CPUs (>8 cores, no throttling)
 * - AVX-512 6x16 micro-kernel when compiled with -mavx512f (NR=16, ZMM regs)
 * - Falls back to AVX2 6x8 (NR=8, YMM regs) otherwise
 *
 * Compile (AVX-512):
...
static void get_blocking(int n, int*pMC, int*pKC, int*pNC){
    ...
    else if(n <= 1024) {*pMC=MC_TINY;  *pKC=KC_LARGE; *pNC=NC_DEFAULT;}  /* KC=320 from sweep */
                               ...
                               double*__restrict__ C, int n)
{
    ...
    /* Table-based thread capping for many-core systems (>8 threads).
       From parameter sweep at each size — formula was suboptimal at 1024. */
    int active_threads = max_threads;
    if(max_threads > 8 && n < 2048){
        int cap;
        if(n <= 256)      cap = 4;
        else if(n <= 512)  cap = 6;
        else if(n <= 1024) cap = 12;  /* sweep: 12T + KC=320 = 163 GF (beats BLAS 161.8) */
        else               cap = max_threads;
    ...
    int enable_thermal = (n >= 4096 && max_threads <= 8);  /* Disable on servers (no throttling) */
                              ...
                              double*__restrict__ C, int n)
{
    int mc_blk, kc_blk, nc_blk;
    get_blocking(n, &mc_blk, &kc_blk, &nc_blk);
    if(n <= 512){mc_blk=MC_TINY; kc_blk=KC_SMALL; nc_blk=NC_DEFAULT;}
    else if(n <= 1024){mc_blk=MC_TINY; kc_blk=KC_LARGE; nc_blk=NC_DEFAULT;}
...
int main(void){
    ...
    printf("====================================================================\n");
    printf("  COM6 v36 - %s, %d threads\n", KERNEL_NAME, nthreads);
    printf("  Sweep-optimized thread scaling + KC tuning\n");
    printf("====================================================================\n\n");
