 ...
 * - Auto-detect thread count from system
 * - Smarter thermal monitoring (higher thresholds, slower to trigger)
 * - C-line prefetching in micro-kernel writeback
 *
 * Compile (AVX-512):
 *   gcc -O3 -march=native -mavx512f -mfma -funroll-loops -o com6_v32 com6_v32.c -lm -lpthread
 *
 * Compile (AVX2 only):
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -o com6_v32 com6_v32.c -lm -lpthread
 */
                               ...
                               double*__restrict__ C, int n)
{
    ...
    /* Smarter thermal: need 5+ iterations, higher threshold (0.55), only for large */
    int active_threads = max_threads;
    double baseline_rate = 0;
    int iter_count = 0, thermal_adjustments = 0;
    int min_thermal_threads = (max_threads + 1) / 2;  /* Don't drop below half */
    int enable_thermal = (n >= 4096);  /* Only for very large (more iterations = more reliable) */
