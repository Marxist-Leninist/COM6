/*
 * COM6 v62 - v26 Proven Blocking + C-Prefetch Kernel + 8192 Support
 * =================================================================
 * v28-v61 had blocking regressions (wrong KC/MC for large sizes).
 * This version goes back to v26's proven 2-tier blocking and adds:
 *   - C-prefetch: pre-fetch output rows into L1 before micro-kernel writes
 *   - 4x unrolled A-packing (from v60)
 *   - 8192 test size (MT-only)
 *   - Isolated CLI mode: ./com6_v62 <size> [mt|1t]
 *   - Cooldown between sequential bench sizes
 *
 * Blocking (proven v26, both fit L2=256KB):
 *   n<=1024:  MC=120, KC=256 → 120*256*8 = 240KB
 *   n>1024:   MC=96,  KC=320 → 96*320*8  = 240KB
 *
 * Compile:
...
/* v26's proven adaptive blocking */
#define KC_SMALL 256
#define MC_SMALL 120
#define KC_LARGE 320
#define MC_LARGE 96
 ...
 * Prefetches all 6 output rows of C into L1 at start, so the final
 * load+add+store at the end hits L1 instead of L2/L3.
 */
...
int main(int argc, char**argv){
    ...
    /* Isolated single-size CLI mode: ./com6_v62 <size> [mt|1t] */
    if(argc >= 2){
        int n=atoi(argv[1]);
        if(n<64||n>16384){printf("Size must be 64..16384\n");return 1;}
        int mode=0; /* 0=both, 1=1t, 2=mt */
        if(argc>=3){
    ...
    printf("====================================================================\n");
    printf("  COM6 v62 - v26 Proven Blocking + C-Prefetch (%d threads)\n",nth);
    printf("  n<=1024: MC=%d KC=%d | n>1024: MC=%d KC=%d | NC=%d\n",
           MC_SMALL,KC_SMALL,MC_LARGE,KC_LARGE,NC);
    printf("  8x unrolled ASM kernel + C-prefetch, 4x A-pack, dynamic sched MT\n");
    printf("====================================================================\n\n");
    ...
    for(int si=0;si<ns;si++){
        ...
        if(n<=512){
            ...
            /* For huge sizes, verify with a second MT run */
            double*C3=aa(nn);
    ...
    printf("\nv26 proven blocking + C-prefetch kernel + dynamic scheduling\n");
    printf("Run individual sizes: ./com6_v62 <size> [mt|1t]\n");
    return 0;
}
