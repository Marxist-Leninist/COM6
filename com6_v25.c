/*
 * COM6 v25 - Deeper KC + Vectorized A-Pack Gather
 * ================================================
 * Changes from v24:
 * 1. KC=320 (was 256): deeper K-dimension = fewer packing passes over B,
 *    better amortization. MC=96 keeps A-panel in L2 (96*320*8=240KB < 256KB).
 * 2. Vectorized A-packing: use 2x _mm256_set_pd to pack 4 values at once
 *    instead of 6 scalar loads. Remaining 2 values are scalar.
 * 3. Keep 8x unrolled ASM micro-kernel from v24.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -static -o com6_v25 com6_v25.c -lm
 */
...
/* 8x k-unrolled 6x8 ASM micro-kernel (from v24) */
...
/* B-packing (contiguous SIMD copy) */
...
/* A-packing — interleave MR rows. For MR=6: gather from 6 rows per k */
static void pack_A(const double*__restrict__ A,double*__restrict__ pa,
    ...
    for(int i=0;i<mc;i+=MR){
        ...
        if(mr==MR){
            ...
            /* 2-at-a-time unroll of the k loop */
            int k=0;
            for(;k+1<kc;k+=2){
...
int main(void){
    ...
    printf("====================================================================\n");
    printf("  COM6 v25 - KC=%d MC=%d + 8x Unrolled ASM (%d threads)\n",KC,MC,nth);
    printf("  Deeper KC for better packing amortization\n");
    printf("====================================================================\n\n");
