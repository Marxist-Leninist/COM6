/*
 * COM6 v44 - Smart Thread Threshold + v43 Base
 * ==============================================
 * n<=256 runs single-threaded (fork overhead > compute at 33M FLOPs).
 * OpenBLAS also uses 1T for small matrices — matching their strategy.
 *
 * 4-tier blocking (unchanged from v43):
 *   n<=512:  MC=48  KC=256 NC=2048
 *   n<=1024: MC=120 KC=256 NC=2048
 *   n<=4096: MC=96  KC=320 NC=2048
 *   n>4096:  MC=96  KC=320 NC=1024 (L3-friendly B panel = 2.5MB)
 *
 * beta=0 memset elimination, C-prefetch, single parallel region.
 *
 * Compile:
 *   gcc -O3 -march=native -mavx2 -mfma -funroll-loops -fopenmp -o com6_v44 com6_v44.c -lm
 */
                           ...
                           double*__restrict__ C,int n)
{
    ...
    /* Don't MT at 256 — overhead kills it. OpenBLAS uses 1T for small. */
...
int main(int argc, char**argv){
    ...
    printf("====================================================================\n");
    printf("  COM6 v44 - Smart Thread Threshold (%d threads)\n",nth);
    ...
    printf("\nv44: 1T for n<=256 (fork overhead > compute), v43 everything else\n");
    return 0;
}
